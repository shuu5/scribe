...
context cycle（`/clear`・respawn）や compaction を「単なるリセット/圧縮」ではなく
「**命令の選択的外部化**」の機会として扱う。

> **運用前提（cycle-first・2026-07-15 改訂）**: 意図的な cycle の正路は `/clear`（+ 各 project の
> resume 正路）または respawn で、手動 `/compact` は cycle 正路から廃止済み。auto-compact の発火は
> **incident** で、compaction フック群はその復元安全網として残置する。cycle 規律そのものの SSOT は
> scriptorium top-spec §1.1 + scribe protocol.md（本 doc は cc-session 側の transport/機構のみを記述し、
> 規律を再定義しない）。
...
ユーザーは 1M でも精度低下のため、context が濁る前に**意図的に cycle する**運用（現行は cycle-first:
`/clear`・respawn が正路。旧運用の ~50% 手動 compact は廃止済み）。cycle の目的は容量でなく
...
`/clear`・respawn に至っては fuzzy も sharp も**全て**捨てる。だから sharp な要素を context の外へ
退避してから cycle する——これが本スキルの存在理由（どの経路でも退避の仕事は同じ）。
...
`session-start-clear.sh`（`matcher: "clear"`）はこの非対称を、**フル自動復元ではなく read-only ポインタ**で埋める安全網（grill 2026-06-23 で確定した運用 (b)＝「基本は /compact、たまの /clear で失わず拾えれば十分」に対応する論点2案B。注: この運用前提は cycle-first 裁定〔2026-07-15〕で主客反転済み——現行は `/clear`/respawn が正路・復元の本線は各 project の resume 正路で、本ポインタは resume 未導入 project のフォールバック。機構の記述は不変に有効）:
