...
- **C-7 bypass = 人間操作のみの多層**。通常=marker 作成、緊急=policy 削除/空化 or `SESSION_ENFORCE_OFF=1` を**ユーザーが**セット。Claude は実行せず提示のみ（git ガードの代替ルート流儀）。
  - **env 経由の silent disable は Position B 限界として受容（`ccs-5p4.2`・won't-fix）**: policy path（`ENFORCE_POLICY_FILE`/`WORKING_MEMORY_DIR`）も `SESSION_ENFORCE_OFF` も env 上書き可能なため、Claude が settings.json の `env` ブロックを編集すれば enforce を silent に無効化できる（policy を存在しないパスへ向ける／bypass env をセット。実機確認: `ENFORCE_POLICY_FILE=/nonexistent` → policy 不在 no-op → 全 allow）。これは env-configurable な opt-in 設計に内在し、かつ settings.json 編集は **deliberate な config 改変＝determined evasion** で、本層が防ぐ「**沈黙の・偶発的**な自己認可」の範囲外（C-4b と同じ Position B 境界）。`SESSION_ENFORCE_OFF` を bypass marker へ置換しても policy path redirect で同じ disable が成立するため marker 化は本質的緩和にならず見送った（adversarial review で実証）。真に塞ぐには policy path の非 env 固定化等の脅威モデルを超える別設計が要る。
  - **機構1/2 の Position B 限界（marker 層の権限分離・署名化は単一 uid モデルでは won't-fix・`ccs-5p4.4`）**: 「marker dir を Claude から書けなくする（権限分離＝機構1）」「marker を署名/TTY 限定で偽造不能にする（機構2）」案は、`cld` が `exec claude --dangerously-skip-permissions` で Claude・hook・人間の生シェルを**同一 uid・同一プロセスツリー**で動かす現モデルでは privilege boundary が原理的に立たない。設計検討（`wf_7bdc49f7`・4 レンズ＋機構別 adversarial 実機検証が収束）で確認: (a) 同一 uid は所有 dir を `chmod`/ACL で自分から剥がせず（owner エントリ優先）`chattr +i` も非 root で不可、(b) 対称鍵 secret/秘密鍵を同 uid で読め署名はバイト一致で偽造可、(c) `[ -t 0 ]` の TTY 検査は `script`/pty で偽装可、かつ marker は空ファイルで直接 touch 可（helper 迂回）、(d) いずれも `ENFORCE_MARKER_DIR`/`ENFORCE_POLICY_FILE` redirect・`SESSION_ENFORCE_OFF`・hook/lib の Edit で空洞化＝上の env-disable と同じ**最弱経路に律速**される。よって marker 層だけ要塞化しても全体保証は上がらず、見た目の強固さが false sense of security を生む（`ccs-5p4.2` won't-fix と同型の論理＝上流に env 全停止経路がある限り下流 marker 層の強化は全体保証を上げない）。真に超えるには enforce 層単体でなく policy/marker/公開鍵パスの**非 env 固定化** + hook/lib の別 uid read-only 化 + Claude を非特権 uid サンドボックスで動かす**実行基盤移行**を要する別フェーズ。なお認可スコープ漏洩（pr-merge のフラグ非区別）は脅威モデルを変えず閉じられる Position B 内の別問題として `ccs-5p4.7` で解消済み（§9.7 末尾）。
...
**残る follow-up（beads epic `ccs-5p4` で追跡）**: なし（epic 完了）。**Position B 限界として受容（won't-fix・`ccs-5p4.4`）**: marker dir の権限分離（機構1）・marker 署名化/TTY 限定 unlock（機構2）＝HIGH#5 の「技術強制への格上げ」は、単一ユーザ・同一 uid モデル（`cld` が `exec claude --dangerously-skip-permissions`）では privilege boundary が原理的に立たず won't-fix（設計検討 `wf_7bdc49f7` で 4 レンズ＋機構別 adversarial 検証が収束。詳細根拠は §9.4 C-7「機構1/2 の Position B 限界」）。**Position B 限界として受容（won't-fix・`ccs-5p4.2`）**: settings.json env 経由の enforce 無効化＝上記 C-7 の通り policy path/bypass env が env 上書き可で、settings.json 編集（determined evasion）により silent disable できるが本層の対象外。marker 化は policy path redirect で同じ disable が成立するため見送り（adversarial review 実証）。**✅ 解消済**: (1) TTL 未指定 gate の無期限化（`ccs-5p4.1`）＝`ep_policy_health` が sha_keyed≠true gate に有限 TTL を必須化し、無ければ corrupt→fail-closed scoped に倒す（ランタイムと同一の `ep_gate_ttl` で判定。Position B）。(2) `.match` schema 検証（`ccs-5p4.3`＋`ccs-5p4.6`）＝probe で `.match` を object・`any_re` を非空の文字列配列に必須化（substring-only の .all 単独 gate と、`any_re`=object 等の沈黙 fail-open を corrupt 化）。`.key`・`.match` 型不正や jq index abort も probe ガード＋ループ rc 捕捉で順序非依存に corrupt へ surface。(3) TTL 値の hardening（`ccs-5p4.5`）＝`ep_gate_ttl` を 10 進正規化（先頭ゼロの8進誤解釈/監査汚染を解消）＋桁数/上限（既定 30 日 `ENFORCE_TTL_MAX_SEC`）ガード（巨大値の事実上恒久 unlock を空→corrupt 化）。(4) pr-merge の危険フラグ keying（`ccs-5p4.7`）＝`--admin` を marker key に折り込み `--squash` unlock の認可スコープ漏洩を解消（§9.7 末尾参照・α=Position B 内）。(5) duplicate gate-id の marker 衝突（`ccs-5p4.8`・pre-existing）＝probe に id 一意性検証を追加し、同一 id の 2 gate（とくに both sha_keyed:true）で良性 unlock が危険操作を認可する漏洩を corrupt→fail-closed scoped 化（§9.7 末尾参照）。**→ epic `ccs-5p4` 完了**。
...
- **[✅ 解消済・`ccs-5p4.7`] pr-merge の危険フラグ keying**: 同一 PR・同一 head SHA でも `--admin`（ブランチ保護/必須レビュー bypass）を marker key に折り込むようにした（`--squash` の unlock が `--admin` を巻き込み認可するスコープ漏洩を解消）。`.key.risk_flags`（allowlist `[{token,match_re}]`）を `ep_marker_risk_suffix` が実マッチ同一エンジン（bash `[[ =~ ]]`）で評価し、ヒット token を `sort -u` で `-flag-<t>` に折り込む（順序非依存）。マージ方式フラグ（`--squash`/`--merge`/`--rebase`）は keying せず＝過剰 gate 回避（認可境界は「PR を head SHA で merge する」ことで、squash か否かはスタイル選択）。`risk_flags[].match_re` は `ep_policy_health` の ERE 検証ループに登録済み（無効 ERE は `[[ =~ ]]` で常に偽になり危険フラグを黙って見逃す＝漏洩の silent 復活になるため corrupt で surface。第2ラウンド CRIT「ERE 検証エンジン乖離」と同型の footgun を回帰固定）。**merge 前 adversarial review（`wf_5b708e0a`）で NO-GO→修正の 3 点**: (1) [CRIT] match_re の末尾境界が狭く語末シェルメタ文字（`--admin;`/`--admin|`/`--admin&`/`--admin>`/`--admin)`）で `--admin` が素 marker に化け hook を rc=0 突破した（出荷 example.json 自体に存在）→ gate 自身の any_re と同じ**両側境界** `(^|[^[:alnum:]_-])(--admin|-A)([^[:alnum:]_-]|$)` へ（`=` を含み `--admin=true` は保持）。(2) [HIGH/MED] 空文字列・改行・タブ入りの ERE 文字列が health=active のまま runtime で silent 失効（health↔runtime 分割乖離・ccs-5p4.1 同型）→ probe で全 ERE セレクタ（`any_re`/`subject_re`/`sha_validate_re`/`risk_flags.match_re`）の空・改行・タブを corrupt 化。(3) [HIGH→根治] free-form subject token gate での marker リテラル衝突。当初は `ep_marker_base` の subject path で予約区切り `-flag-`/`-sha-` を deny したが、**第2ラウンド review（`wf_afb1a6e0`）が NO-GO**: deny は subject 経路しか塞がず、衝突は gid/prefix のフラット `-` 連結経路へ移っただけ（別 gate が同一 readable marker を生み、良性操作 1 つの unlock が危険操作を巻き込み認可＝実 hook で exit 0 を再現。largely pre-existing な marker 文法の曖昧さ）。さらに subject deny は `feature-flag-x`/`fix-sha-mismatch` 等の正規ブランチ名を恒久 deny する over-block 副作用があった。→ **根治（marker 文法の曖昧性除去）**: marker を `<gid>-<slug(prefix)>-<subject><risk><sha>-<disamb16>` 形へ変更。`disamb16` = 構造化フィールド（gid・strategy・slug(prefix)・subject・risk・sha）を**改行区切り**（全フィールド改行不含＝直列化が単射）で並べた sha256 の先頭 16 桁。異なる操作タプルは必ず別 marker＝**構造的に衝突不能**（readable 部は監査用に保持）。これで `ep_marker_base` は `ep_marker_name` へ統合（disamb と readable を同一フィールドから導出し drift 排除）、subject runtime deny（と over-block/dead-end/誤誘導 message）を撤去し、新規（`-flag-`）と pre-existing（gid/prefix・`-sha-`）の両衝突を一掃。あわせて `subject_prefix` を slug 化＋health で形式検証（reviewer 指摘の無検証を解消）。**残存限界（Position B 外・明示）**: 変数間接（`f=--admin; gh pr merge 3 $f`）・語内 IFS 分割（`--ad${IFS}min`）・コマンド置換（`$(echo --admin)`）の admin は実行前の文字列に `--admin` が現れず検出不能。フラグ keying を入れた途端「admin を外す最も自然な回避が変数間接」になるが、これは上記[残存・脅威モデル外]・§9.4 C-7 と同じ regex-on-command-string の原理的限界で本層の対象外（「塞いだ」と誤認しないこと）。
- **[✅ 解消済・`ccs-5p4.8`] duplicate gate-id の marker 衝突（pre-existing）**: `ccs-5p4.7` R3 review（`wf_9b7027ca`）が CONFIRMED した pre-existing fail-open。policy が同一 `id` の 2 gate を持つと、marker は `<gid>-...` で導出され名前空間を gid で共有するため、良性 gate 用の unlock marker を危険 gate が流用できる（認可スコープ漏洩）。とくに**両 gate が `sha_keyed:true`** だと no-TTL gate の TTL ループを免除され `ep_policy_health=active` のまま到達した（非対称 key の dup で `ep_gate_field` が単一行化し marker 派生が成功＝良性 `--squash` と危険 `--admin` が同一 marker `pr-merge-pr-N-<disamb>` に衝突。良性 unlock→危険操作が**実 hook exit 0** で再現）。disamb hash は gate 内容（gid/strategy/prefix/subject/risk/sha）から導出され、gid 共有そのものは構造的に救えない。→ **根治: `ep_policy_health` の probe に id 一意性検証**（`(.gates|map(.id))` の `length != (unique|length)` → corrupt → fail-closed scoped）を追加。id 形式検証（`^[a-z0-9-]+$`）が先行するので `unique` は非 null 正規文字列に対し安全。あわせて `/session:enforce`（SKILL.md）の `id` フィールドに**一意必須**を明記（authoring 時に別 id を割り当て、Step 0 の既存 id 一覧と衝突させない）。回帰: health（symmetric dup＋R3 非対称 PoC が corrupt）・hook e2e（dup-id で良性 unlock 拒否＋危険操作 exit 2）。**残存特性（C-6 既存仕様・本変更の責務外）**: corrupt→fail-closed scoped は内蔵 danger list（`ENFORCE_BUILTIN_DANGER_REGEX`＝`gh pr merge`/`git push`/`git merge`/`kubectl|helm apply|install|upgrade`/`terraform apply`/`serverless deploy`）**以外**の custom 危険語彙を gate にしている場合、dup-id 等の corrupt で当該 custom gate が degraded（allow）に倒れうる。ただし unlock helper も corrupt で marker 作成を拒否する（rc=66）ため「沈黙の自己認可」ではなく degraded enforcement＝運用者は health=corrupt を surface され policy を直すべき（bad JSON・無効 ERE・TTL 欠落と同一チャネル）。**これで enforce hardening epic `ccs-5p4` は完了**（Position B 内の既知 fail-open をすべて解消）。
