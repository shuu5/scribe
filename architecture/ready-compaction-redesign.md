...
**残る follow-up（beads epic `ccs-5p4` で追跡）**: settings.json env 経由の恒久 OFF（C-7 の単一 env 依存・`ccs-5p4.2`）、TTL 値の hardening（巨大値の事実上恒久 unlock・先頭ゼロ8進・`ccs-5p4.5`）、marker dir の権限分離による hard 化（HIGH#5 の技術強制への格上げ・`ccs-5p4.4`）。**✅ 解消済**: (1) TTL 未指定 gate の無期限化（`ccs-5p4.1`）＝`ep_policy_health` が sha_keyed≠true gate に有限 TTL を必須化し、無ければ corrupt→fail-closed scoped に倒す（ランタイムと同一の `ep_gate_ttl` で判定。Position B）。(2) `.match` schema 検証（`ccs-5p4.3`＋`ccs-5p4.6`）＝probe で `.match` を object・`any_re` を非空の文字列配列に必須化（substring-only の .all 単独 gate と、`any_re`=object 等の沈黙 fail-open を corrupt 化）。`.key`・`.match` 型不正や jq index abort も probe ガード＋ループ rc 捕捉で順序非依存に corrupt へ surface。
