...
| 境界 | レイヤ | 実装 |
|---|---|---|
| Bash subprocess の write → worktree + 許可 path のみ | OS 外壁（bwrap・層2） | `gen-sandbox-settings.sh`（allowWrite は `.beads` runtime サブパス + 自リポの bdw flock 鍵 **file** + worker の git commit write-set〔私有 gitdir + 共有 objects/refs/logs・sc-kxec で明示 grant 化〕へ最小化・OG-1/sc-nd6 + OG-4/sc-mcx + sc-kxec） |
...
- `gen-sandbox-settings.sh <worktree> [<anchor>]` — worker 用 `.claude/settings.local.json` を stdout 出力。
  ...
  **file**(`${lock_dir}/bd-write-<repo_id>.lock`・`bdw lock-file` に問い合わせて byte 一致・OG-4/sc-mcx。
  lock dir 丸ごとを開けず同 dir 内の他リポ鍵に触れない) と worker の git commit write-set（私有 gitdir +
  共有 `objects`/`refs`/`logs`・`$wt` から `git rev-parse` 動的導出・sc-kxec）を絶対パスで入れる。
  cwd(worktree) は sandbox 既定で writable(列挙不要・CC 既定)。共有 `.git` は **旧: CC 既定 writable に依存** して
  いたが、この既定が CC version で不安定に変わり commit が silent に非永続化した incident（sc-ghjc・2.1.217）を
  受け、**commit write-set を明示 grant** して CC 挙動から決定論化した（`.git/config`・`.git/hooks`・`.git` 直下は
  列挙しない＝un-1n1 fence を denylist なしに構造保証）。
...
- **git commit → 共有 .git に永続**（linked worktree の commit write-set＝私有 gitdir + 共有 `objects`/`refs`/`logs`
  は cwd の外だが、gen が **明示 allowWrite grant** する〔sc-kxec〕。旧前提「CC sandbox 既定で writable・列挙不要」は
  CC version で不安定化したため廃止＝この e2e は gen 直呼びゆえ **明示 grant 経由の永続**を実証する。CC 2.1.217 で
  PASS 実証済＝明示 grant が 2.1.217 の confinement を貫通して commit を永続させることの live 裏取り）。
...
- **counterfactual 負 control（sc-1d95）→ .git grant 4 集合（私有 gitdir + 共有 objects/refs/logs）を strip した変種
  settings で 2nd sandboxed commit を試み、共有 .git の HEAD が不変（before==after）であることを assert する**。これで
  「.git 明示 grant は load-bearing（無ければ commit は永続しない）」を counterfactual 実証し、正 control（明示 grant
  *経由で* 永続）の boundary-vacuous を閉じる。判定は **rc でなく HEAD 比較**（CC 2.1.217 の silent rc=0 消失にも
  2.1.211 系 loud EROFS にも同一判定式が成立）。strip しても commit が永続してしまったら「CC default auto-grant 復活の
  疑い（CC version drift）」で **loud FAIL**（どちら向きの drift も fail-closed＝silent 化の再演防止）。strip は jq で
  allowWrite から共有 .git 配下 path のみ除去し **.beads/lock grant は残す**（「sandbox 全体の故障」でなく「.git grant
  の欠如」だけを isolate）。
- **CC version 自己捕捉（sc-1d95）→ 出力ヘッダに `claude --version` の実測値を 1 行出す**（`--- CC version: X.Y.Z ---`）。
  PASS/FAIL どちらの記録にも version が in-band で残り、将来の drift 調査で attribution（どの CC で観測されたか）が
  out-of-band 情報に頼らず独立に成立する。
...
```
bash scripts/sandbox-spike/verify-sandbox-e2e.sh          # 単発（deps 要・FAIL=0 が green。counterfactual は CC が auto-grant する版では DRIFT=1 と表示（rc 非影響・sc-1d95 裁定改訂）・load-bearing 確認できる版では PASS 加算）
