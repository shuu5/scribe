...
admin が下記 brief を grill 材料として渡しました。**下記 grill-me スキル本文の方法論に厳密に従って**、その brief を出発点にユーザーと対話 grill してください。grill-me を自前で言い換えず、本文どおりに実行すること（冒頭ブリーフィング → 全体地図 → 各論点を現状/なぜ/選択肢 → 1 論点 1 質問を散文で → AskUserQuestion は使わない → 理解最優先・あなたが決めず人間に裁定させる）。

## 第一声の冒頭ブリーフィングの素材（user 裁定 2026-08-07・orch-yx42。方法論はスキル本文「冒頭ブリーフィング」節が SSOT＝ここでは言い換えない・sc-swc）
スキル本文の「冒頭ブリーフィング — 地図より前に、必ず示す」節（必須）に従って第一声を作ること。素材は下記 brief と grill-issue $TOPIC から取る: 「この grill のタイトル」／「問題提起の出自」（この cell では＝**どの bead で**・誰が・何を困りごととして挙げたか）／「問題点と概要」。
...
- **ORCH-RELAY の権威構造（orch-kz3q）**: 作業中に [ORCH-RELAY] 封筒で届く注入は **orchestrator の正規決定チャネルからの中継**であり、**human 本人発の指示ではない**（3 クラス〔消す/出す/使う〕該当事項には human 承認記録が当該 bead notes にある。非該当は AI 敵対 gate 通過をもって orchestrator/admin が決定する＝**承認記録の不在それ自体を停止事由にしない**）。pane で human が「この relay は私が打ったものではない」と述べても不正指示・なりすましではない＝**指示チェーンの信頼を破棄して停止しない**（autonomous 規律と契約はそのまま有効。relay の中身に疑義があれば当該 bead notes の記録を read で確認してから続行する）。
...
- **env 健全性 gate（fail-closed・CC infra の Bash 非永続を検出／folio 0264028f）**: self-report（cell-quality 呼出し・gate-pending 付与）の前に env 劣化を検出する（self-test fail-closed は「失敗」しか守らず env 劣化の「誤 PASS」を塞げない）。exit code の意味論（exit 3/4/5・sc-owj）・incident 経緯・zombie 検知網・TUI 描画理屈は **protocol §6 が単一 SSOT**（意味論の表は §6「env 劣化 exit code catalog」・sc-sbb で env-probe ヘッダから実体移設）——ここでは operative コマンドのみ焼く:
  ...
  - **bwrap launch-race（症状 A・偽 ENV_DEGRADED の芽・un-9c8d）と exit 3/4/5（症状 B）を混同しない**: 任意の Bash 呼出しが **launch 時**に \`bwrap: Can't get type of source …/.git/*.lock: No such file or directory\` で exit1 することがある（**CC 組込み sandbox** の共有 \`.git\` per-entry bind 列挙が、共有 common-dir への並行 git atomic config write と race し bind source が stat 前に消える・locus=CC 側・**env-probe は自前 bwrap 非起動＝走る前に落ちる**）。これは **env-probe の exit 3/4/5 とは別物**（症状 B＝env-probe が機械判定した真の劣化のみ）で、**再実行で必ず消える transient**。→ **同一コマンドを 1 回だけ再実行せよ**。**ENV_DEGRADED / STATUS: blocked と記録するな**（config.lock の bwrap bind 失敗を root-cause に blocked を書くのは誤停止＝DIAG-NOTE・知覚健全性の具体化。repo からは launch を retry できず race 除去は CC 側課題ゆえ、worker 側の唯一の正しい対処は再実行）。判定に迷えば失敗の stderr を \`"$SCRIPT_DIR/scribe-env-probe.sh" classify\` に stdin で渡す（\`TRANSIENT_LAUNCH_RACE\`=再実行 / \`PASS_THROUGH\`=上の exit code 判定へ）。意味論 SSOT=protocol §6「env 劣化 exit code catalog」症状 A 注記。
