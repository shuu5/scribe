...
## 第一声の冒頭ブリーフィング（user 裁定 2026-08-07・orch-yx42・grill-me 全体地図より前に置く）
起動直後の最初の応答では、grill-me の全体地図へ入る**前に**、下記 brief から次の 3 点を平易に要約提示すること: (1) この grill のタイトル (2) 元の問題提起＝どの bead で・誰が・何を困りごととして挙げたか (3) 問題点と概要。地図だけでは問題提起の中身が読み手に伝わらない（複数 project の consult を並行する運用での実測）。この要約は「AI がどう問題を理解したか」の提示＝ユーザーとの理解ズレ検証面を兼ねる。要約のあとに grill-me 本文どおり全体地図へ進む。
