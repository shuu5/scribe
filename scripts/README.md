...
テスト: `tests/scribe-tools.bats`（dry-run arg-echo を assert・実 spawn しない。bd は `tests/fixtures/bd-stub.sh` でスタブ）。`tests/scribe-inject.bats`（送達確認の残留検知を fixture capture で assert・tmux は `SCRIBE_TMUX` stub・**実 tmux 起動しない**）。worker 自身の fail-closed self-test は `selftest-<id>.local.sh`（untracked）。
