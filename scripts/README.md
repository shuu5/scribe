...
admin / worker の手作業（protocol.md §1/§2/§5）を 1 コマンド化する 4 点。**全 script に `--dry-run`**（実行するはずのコマンド列を arg-echo・実 spawn はコスト大）。
...
→ scribe-design.md §14 の「道具」3 本柱（spawn / gate-args / cleanup）に対応。worker 自己点検の `scribe-selftest-args.sh` は un-3yc で追加した 4 点目（protocol.md §2 の自己点検 args を 1 コマンド化）。
