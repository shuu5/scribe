...
- `scribe-inject.sh {verify,send}` — admin 操舵注入の**送達確認**ヘルパ（sc-6vj）。2026-07-08 に操舵注入の送達失敗が 2 度実発生した（bracketed-paste 滞留・`-l` 逐字滞留を pane 目視で誤『送信済み』判定）ことへの対策。`verify` は capture-pane 出力（`--capture-file`/stdin）と注入 payload の `--marker` を受け、CC TUI 入力ボックス *内部* に marker が残るか（=未送達か）を機械判定する（送信済みで transcript に echo された自分のテキストとは弁別）。`send --target <pane> (--file|--text)` は send-keys -l + Enter → capture → verify し、残留なら Enter を追送して回復、retry 尽きても残留なら **fail-loud**。終了コード: 0=DELIVERED / 3=RESIDUAL / 4=INCONCLUSIVE（入力欄特定不能＝保守的 fail-loud）。tmux バイナリは `SCRIBE_TMUX` で差し替え可（テスト seam・live server 非依存で回帰テスト）。設計判断（tmux の after-send-keys hook / global pipe-pane は見送り）の rationale は script ヘッダが SSOT。
