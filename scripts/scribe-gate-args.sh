...
# 旧 case-sensitive `*fable*` は大文字混在を見逃す fail-open だった（sc-vuu facet4）。
# `scribe-spawn.sh` / `scribe-selftest-args.sh` の同 `case "${MODEL,,}" in *fable*` イディオムと
# 統一（3 兄弟対称化。行番号はドリフトするためスクリプト名で参照＝sc-1sc）。
...
ACCEPTANCE="$ACCEPTANCE" MODEL="$MODEL" CTX_FILE="$CTX_FILE" python3 - <<'PY'
import json, os
