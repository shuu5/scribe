...
# effort（sc-7ac・sc-dc9 申し送り）: env CLAUDE_CODE_EFFORT_LEVEL（spawn が worker env へ注入する実効 effort）を
#   読んで args.effort へ焼く。フラグではない（worker 実効値を追随させる派生ノブ）。allowlist(low|medium|high|
#   xhigh|max)外・未設定は焼かず WF 側 fail-safe（既定 high）に委ねる。
#
...
case "${CLAUDE_CODE_EFFORT_LEVEL:-}" in
  low|medium|high|xhigh|max) EFFORT_LEVEL="$CLAUDE_CODE_EFFORT_LEVEL" ;;
esac
