...
#   --review-effort LVL   review 段 effort の opt-in（既定: 未指定=WF 既定 high）。allowlist 完全一致
#   --verify-effort LVL   verify 段 effort の opt-in（既定: 未指定=WF 既定 high）。allowlist 完全一致
