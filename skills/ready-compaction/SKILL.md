...
description: |
  ...
  横断/インシデントの事実は doobidoo、hard 候補はマークのみ。
...
| **hard 候補**（gate を持ち歪みを許せない命令）| working-file に `[hard候補]` マーク | マークのみ（実強制は Phase-2 hook）|
...
| **hard 候補**（gate-point を持ち、わずかな歪みも許したくない命令）| working-file | 「命令・制約」節に `[hard候補]` タグでマーク（実強制は Phase-2 の hook、ここではマークのみ）|

> **hard 候補は適用範囲（always/default/effort）に依らずマーク対象**（`any × hard`）。effort 項目に限定されない。実強制は Phase-2 の PreToolUse hook で、本スキルはマークのみ。
