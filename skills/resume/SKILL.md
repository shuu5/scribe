...
name: resume
description: |
  respawn / compaction 後の第一手として、resume DATA（WM主張↔bd現在値の乖離 / orphan WM / auto-compact 強制回復 mode /
  ...
  機械層（fetch）= scripts/scribe-resume-fetch.sh が read-only で DATA を出し、判断（brief）と consume（mv）は本 skill が担う。
  ...
  says 「resume」「復元して」「続きから」「respawn した」「compaction 後の再開」「作業状態を戻して」。
---

# scribe:resume Skill（respawn / compaction 後の復元 brief）
...
- **機械層 = `scripts/scribe-resume-fetch.sh`**（read-only・一切 write しない）: DATA を行頭 marker 付きで emit する。
...
```
"${CLAUDE_PLUGIN_ROOT}/scripts/scribe-resume-fetch.sh"
```

cwd の repo（`.beads/` を持つ root へ walk-up）を anchor とし、その台帳・その `.claude-session/` を読む。
別 repo を対象にするなら `SCRIBE_RESUME_ANCHOR=<repo root>` を付ける。session id が env から解決できない場合は
`SCRIBE_RESUME_SID=<sid>` を渡す（未解決だと legacy 非 scoped WM を見に行き、current の WM を取り違える）。
...
| anchor を解決できない | cwd が `.beads/` を持つ repo の外 | 対象 repo で実行するか `SCRIBE_RESUME_ANCHOR` を渡す |
...
- `[RESUME-MODE] normal | force-recovery` … `force-recovery` は auto-compact marker 検出＝**WM の退避が間に合わなかった疑い**。
...
2. 採用した候補の sid で **再 fetch** して bd と突合する（これで `[DIFF-*]` が実データになる）:
   ...
   ```
   SCRIBE_RESUME_SID=<候補の sid> "${CLAUDE_PLUGIN_ROOT}/scripts/scribe-resume-fetch.sh"
   ```
...
→ 機械層の env seam・marker 仕様・orphan 規則の SSOT は `scripts/scribe-resume-fetch.sh` の header。
