...
> consult 起動は 3 クラス（消す/出す/使う）**非該当**（何も消さず・何も公開せず・追加課金も発生しない＝定額プラン内の token 消費のみ）ゆえ
> **AI 判断で起動する**（起動前の承認確認は取らない）。ただし **起動する理由を 1 行開示**してから実行する
> （新セッションが立つ事実を人間が事後に追える可視性は残す）。承認体制の裁定 SSOT のポインタ = bd sc-tx8s。
