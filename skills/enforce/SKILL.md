...
- `id`: `[a-z0-9-]+`（例 `pr-merge` / `git-push` / `deploy`）。**gate 間で一意必須**。id は marker
  名前空間のキー（marker は `<id>-...` で導出される）なので、重複すると別 gate が同じ marker を共有し、
  良性 gate 用の unlock が危険 gate を認可しうる（認可スコープ漏洩）。重複は health が corrupt 化して
  fail-closed scoped に倒すが（ccs-5p4.8）、**authoring 時点で別 id を割り当てる**こと。`/session:enforce`
  の Step 0 が既存 id を一覧する（下案の id と衝突させない）。
...
# 承認された gate（GATE_JSON）を追加（atomic: tmp → mv）。id が既存 gate と重複しないこと（一意必須）。
