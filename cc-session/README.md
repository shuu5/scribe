...
## canonical ソース（subtree）と subtree pull 時の注意

この `cc-session/` は **session plugin の canonical live carrier**（実装の正典・実稼働コピー）である。`~/.claude/plugins/session` はこの subtree を指す（実測: `~/.claude/plugins/session -> <repo>/cc-session` の symlink）。開発・修正・レビューはすべて本 subtree で行うこと。

- standalone の cc-session リポジトリ（例 `~/projects/local-projects/cc-session`）は **凍結/乖離** した派生であり、canonical ではない（実測差分あり: standalone `scripts/session-comm.sh` = 75075B(Jul20) に対し本 subtree = 78037B と分岐）。ここへ書いた変更は上流に反映されない。
- **subtree pull 時の footgun 再流入 watch（Footgun-2）**: pipefail 下の `producer | grep -q` は、`grep -q` がマッチで早期 exit し producer が SIGPIPE 死すると pipeline rc=141 になる。これが `if` 条件だと「present→absent」の誤報となり、`session-comm.sh` の送達判定では **fail-open**（偽 `DELIVERED` / dialog 空 Enter の自動確定）へ倒れる。本 subtree はこの latent site を herestring 化（`grep -q ... <<< "$var"`）で hardening 済みだが、standalone からの subtree pull / merge で旧 `printf ... | grep -q` 形が再流入しうる。**subtree pull 後は `session-comm.sh` に対し Footgun-2 sweep（`printf '%s' … | grep -q` 形の否定条件・送達判定経路の再点検）を再実行**し、再流入した pipe 形を herestring へ戻すこと。
