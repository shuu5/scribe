...
description: |
  ...
  （Working Memory ファイル）だけを自前で運ぶ。恒久命令は bd 起票（notes に audience / channel / 毎 boot 必須か / 根拠を必須記載）で昇格提案、
...
|---|---|---|
| 恒久命令（このリポで常に真）| **bd 起票**（どの carrier へ載せるかは起票 notes で審議）| 「bd 起票しては?」と**提案のみ**（notes に audience / channel / 毎 boot 必須か / 根拠を必須記載。carrier の確定は起票後）|
...
| 判定 | 振り分け先 | アクション |
|---|---|---|
| **恒久命令**（このリポで何をしていても常に真の手法・規約）| **bd 起票** | 「**bd 起票**しては?」と**提案**（notes に audience / channel / 毎 boot 必須か / 根拠を必須記載。下記の配線チェックリストを通す）。どの carrier へ載せるかは**起票後の審議で決める**——スキルは特定 carrier への追記を自ら誘導せず、勝手にコミットもしない。**ユーザースコープのグローバル CLAUDE.md は対象外**——横断的ルールに見えても、グローバルへは「手動で検討を」と口頭で促すだけ（自動提案・自動編集しない）|
...
> **配線チェックリスト**（恒久命令を bd 起票へ回すとき、起票 notes に次の 5 点を書く。埋まらない点が 1 つでもあれば昇格しない）:
> ① 毎 boot 必須か（常時注入が要るのか、必要時 Read で足りるのか）② audience は最小か（誰に効く命令か）
> ③ 既存 carrier・hook 注入と重複しないか ④ 期限はあるか（暫定なら失効条件）⑤ bd 起票 ID（起票後に記入）

<!-- 暫定形: 恒久命令を bd 起票へ回す本経路は暫定。恒久形は orch-22jj.15（context-unit store + compiler）＝store land 後に本実装から移行する。 -->
...
- **プロジェクト固有の恒久知識は doobidoo でなく bd 起票**へ回す（Step 1 と同じ扱い＝配線先はその起票 notes で審議する）
  （常時注入される carrier へ足すと毎 boot のコストを全セッションに課すため、既定の置き場としない）
...
```
✓ 恒久命令 → bd 起票への昇格を提案（notes に audience / channel / 毎 boot 必須か / 根拠を記載・配線チェックリスト通過分のみ）
...
- **working-file に恒久知識を入れてはならない**（恒久命令→bd 起票、恒久事実→doobidoo/git）
- **MEMORY.md を carrier に使ってはならない**（machine-local のため別マシンで stranded する）
- **ユーザースコープのグローバル CLAUDE.md を自動提案・自動編集してはならない**（恒久命令の昇格提案先は bd 起票であり、CLAUDE.md〔プロジェクト / グローバル〕へスキルが直接誘導しない）
