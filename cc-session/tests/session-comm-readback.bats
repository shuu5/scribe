...
# =============================================================================
# ccs-oq9 / orch-55zt: 実測1（「state=processing after 10s・積極証拠なし」＝偽 exit 4 →
#   cld-spawn 内部 retry → 二重送達）の根治。
#   機序: (A) が「隣接 2 poll」を要求するため、実 turn が回っていても描画過渡フレーム
#   （repaint 途中でスピナー行が欠ける / 入力欄 box 未描画＝interior 特定不能 / poll 位相ずれ）が
#   1 枚挟まるだけで streak が落ち、budget 全域で 2 連続を取り逃す（重量宛先＝大 context・
#   大量ストリームで確率が上がる。admin repro の軽量宛先 2/2 では非再現＝happy path）。
#   修正: **宛先が processing を報告し続けている間だけ** streak を保持（gap 許容）。
#   受理条件そのものは不変＝turn 固有マーカーの baseline 新規行を 2 回観測すること。
#   ★discriminator: 直上の :233 pin（同じ振動 frame 系列・state=input-waiting）は依然 exit 4。
#     「宛先が実際に turn を回している」ことだけが gap 許容の入場条件。
# =============================================================================

@test "ccs-oq9 red→green: 描画過渡フレームを挟む強マーカー振動でも processing 継続中なら受理＝exit 0（実測1 偽陰性の除去）" {
    # frame 系列: strong → (repaint 過渡＝box も強マーカーも無い) → strong …（MOCK_PANE_ALT の振動）
    # 宛先は turn 実行中＝detect_state は processing を返し続ける（実測1 のシグネチャ）。
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE="$STRONG_PANE"
    export MOCK_PANE_ALT="noise"
...
@test "ccs-oq9 red→green: 現行 TUI スピナー行（timer-spinner）の振動でも processing 継続中なら受理＝exit 0" {
    # esc to interrupt 不在の現行 TUI（ccs-pwr）でも同じであること。強マーカーは timer-spinner のみ。
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE="$CUR_TUI_PANE"
    export MOCK_PANE_ALT="noise"
...
@test "ccs-oq9 敵対: 強マーカー単発＋processing 継続では受理しない（証拠 2 回が不変・単一 spurious edge の封鎖）" {
    # gap 許容が「1 回の観測 + processing」で受理する fail-open へ退化していないことの mutation 検出。
    # frame: strong（1 枚だけ）→ 以降ずっと noise。state は processing のまま。
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE="$STRONG_PANE"
    export MOCK_PANE_AFTER_N=3; export MOCK_PANE_AFTER="noise"
...
@test "ccs-oq9 敵対: boot 語彙スピナー（Loading…/Baking…）は processing 継続でも受理しない（boot-race 非復活）" {
    # boot splash は timer-spinner を出さない（100 frames verified）が、state は fallthrough で
    # processing を返しうる。gap 許容が boot 経路の fail-open を再開していないことを pin する。
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE=$'Baking…\nInitializing…\n'"$EMPTY_BOX"
    export MOCK_PANE_ALT="Loading…"
    run bash "$COMM" inject-file "session:0" "$PROMPT_FILE" --wait 5 --confirm-receipt 2
    [ "$status" -eq 4 ]
}

@test "ccs-oq9 敵対: タイマー括弧付き boot 語彙でも box 未描画なら processing 継続で受理しない（interior 前提の維持）" {
    # :550 pin の processing 版（boot splash は入力欄 box を描かない＝xrc!=0 で証拠に数えない）。
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE=$'✽ Loading… (2s)\n✽ Connecting… (3s)'
    export MOCK_PANE_ALT="noise"
...
@test "ccs-oq9 敵対: baseline に居る静的 compaction 語は processing 継続でも受理しない（baseline 行差分の維持）" {
    # baseline に写った transcript の一般英単語（Summarizing 等）で gap 許容が発火しないこと。
    export MOCK_STATE=processing
    export MOCK_BASELINE=$'Summarizing the build logs before the change\nmore transcript'
    export MOCK_PANE=$'Summarizing the build logs before the change\nmore transcript\n'"$EMPTY_BOX"
    export MOCK_PANE_ALT="noise"
...
@test "ccs-oq9 敵対: prompt 本文の強マーカー語が入力欄に残留しているだけでは processing 継続でも受理しない（interior 除外の維持）" {
    # 未 submit の残留（RESIDUAL）が gap 許容で受理へ化けないこと（(A) は outside view のみ）。
    printf 'Summarizing logs from the build\n' > "$PROMPT_FILE"
    export MOCK_STATE=processing
    export MOCK_BASELINE=""
    export MOCK_PANE=$'╭──────────────╮\n│ ❯ Summarizing logs from the build │\n╰──────────────╯'
...
@test "ccs-oq9 敵対: state 取得失敗（unknown）は gap 許容の入場条件を満たさない＝振動は非受理" {
    # gap 許容の入場条件は **processing のみ**。state 取得失敗（unknown）や idle まで許容へ広げると、
    # 「宛先の実行状態が分からない」frame を turn 継続の根拠に使う fail-open になる。
    export MOCK_BASELINE=""
    export MOCK_PANE="$STRONG_PANE"
    export MOCK_PANE_ALT="noise"
    cat > "$SANDBOX/mock_scripts/session-state.sh" <<'STATE_EOF'
#!/bin/bash
if [[ "$1" == "wait" ]]; then exit 0; fi
if [[ "$1" == "state" ]]; then exit 1; fi   # 取得失敗＝呼出側は unknown へフォールバック
exit 0
STATE_EOF
    chmod +x "$SANDBOX/mock_scripts/session-state.sh"
    run bash "$COMM" inject-file "session:0" "$PROMPT_FILE" --wait 5 --confirm-receipt 2
