...
- `scripts/` — セッション管理スクリプト群（`cld`, `cld-spawn`, `cld-fork`, `session-state.sh`, `session-comm.sh`, `session-comm-backend-tmux.sh`, `session-name.sh`, `window-manifest.sh`, `claude-session-save.sh` 等）＋ `enforce-unlock`〔hard 強制の marker を人間が生シェルで作る helper〕
  - `session-context-meter.sh` — session context 使用量（used%・絶対 tokens・window size）の **read-only meter primitive**（courier orch-xstc / bd ccs-ehk・fleet-cap 等の外部 consumer 向け canonical）。primary = pane statusline line2 parse（入力ボックス〔最後の `❯` 行〕より下のみ探索・`❯` 不在時は末尾 6 非空行 fallback・健全性 bound・claude 非稼働 pane は pane 厳密の stale-screen gate で不信頼化。claude 判定は cmd==claude ∨ pgid 内 comm=claude の 2 経路＝cld-spawn の wrapper bash pane 対応。bare session 名は「claude が走る唯一の pane」を対象に採り 0 件/複数件は fail-closed。colon 形は session 部の exact 一致 + window 実在照合＝tmux の prefix 一致 / 不在 index の silent fallback による誤帰属を排除）／ fallback = transcript jsonl の最新非 sidechain assistant usage 和（「最終観測値」・% 不能・usage 全 0 の synthetic entry は skip・sid 解決の pane-map 候補は `SESSION_METER_PANE_MAP` > `SESSION_MAP_DIR` > legacy `~/.local/state/claude`〔ubuntu-note-system claude-session-save.sh 由来〕）。出力は 1 行固定順 key=value・不明値 `-`・**非 0 exit は consumer が fail-open（cap 未達＝no-action）にする契約**。詳細契約と env seam は script header が SSOT。
  - `session-context-meter-capfmt.sh` — fleet-cap seam 適合 **adapter（thin shim）**: `ORCH_FLEETCAP_METER_CMD` の契約（単一 word command + 位置引数 1 個・stdout 1 行「`<pct> <abs_tokens>`」両整数）へ canonical meter を橋渡しする。計測 target は consumer の action 対象と一致する **`<session>:$SESSION_METER_WINDOW`（既定 `admin`）** を `--source pane` 固定で測る（bare session の heuristic へ落とさない＝別 window の値を返す false-high を構造排除・jsonl は seam で構造的に捨てられるため走らせない）。used_pct/used_tokens の 2 値のみ stdout・片欠測は 0 埋めせず非 0 exit（consumer は cap unknown＝no-action）・監査痕跡として meter の key=value 行を stderr へ echo。rich な key=value 契約は canonical 側に温存。
