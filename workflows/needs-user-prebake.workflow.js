export const meta = {
  ...
  description:
    '並列 read-only 多面調査 → 統合 brief の凍結骨格（admin が回す）: 相互独立な調査軸(facet)ごとに read-only agent が「現状調査(事実/推測を区別)→決定木→選択肢+トレードオフ→起票候補」を構造化 brief で返し、opus が単一の構造化 brief へ統合して呼出元に返す。recon-brief / 着手前の現状把握 / 影響調査 / 設計オプション洗い出し / needs-user タスクの pre-bake など「実装・決定の前に多面の read-only 調査を束ねたい」場面全般に使う(needs-user 専用ではない)。grill は含まない(対話 grill は grill-consult が別途行う=protocol §7)。F2 構造解消: WF は admin にデータを返すだけで自己 pre-bake を grill しない。固有物(taskRef/taskTitle/anchor/facets/model/roAgentType)は args で差し込む(骨格は再利用)。',
  whenToUse:
    '複数の相互独立な軸で read-only 調査・分析を並列に回し、単一の構造化 brief へ統合したいとき全般。典型: (a) recon-brief=着手前の現状調査・影響調査・設計オプションの洗い出し(調査→決定木→選択肢+トレードオフ), (b) needs-user タスクの pre-bake=「3 クラス(消す/出す/使う)該当、または複数の妥当な設計が併存し選択が人間の目的・価値観に依存する論点(承認要求ではなく grill 提案として上げる型)」の brief 化(返り値の brief を admin が file へ書き grill-consult へ `scribe-spawn --consult --context <brief> <grill-issue>` で渡す=protocol §7), (c) 主張・仕様・契約の多面事前検証。1 facet なら admin インラインで足り fan-out 不要。roAgentType は read-only 段の agentType 上書き escape hatch(既定 scribe:explore・"none" で agentType 無し強制)。',
