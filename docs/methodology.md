...
>
> **「転記でなく参照」の規律**: 品質パターンの**機械的フック**（`agent()`/`parallel()`/`pipeline()`/`schema`/loop の書き方）は **Workflow tool 自身が SSOT**、**凍結された適用形**は `~/.claude/workflows/cell-quality.workflow.js` が SSOT、**WF agent の段別 model/effort ルーティング**は **本書 §1.1 が SSOT（sc-41b で確定。global 配布物側の縮退・back-pointer 反転は連動タスクで実施予定）**、**D1-D7 の決定全文**は doobidoo `e5d79cc9` + 設計 why は scribe-design.md §18 が SSOT。本書はそれらの実体を**転記せず**、「いつ・どの強度で・どのパターンを選ぶか」という判断の方法論だけを蓄積する（蓄積庫であって複製庫ではない＝ドリフト防止）。
>
...
**WF agent 段別 model/effort フルマッピング（本節が SSOT・sc-41b で確定。global 配布物のモデル階層ルーティング節の縮退・back-pointer 反転は連動タスクで実施予定＝現時点では global 節も本マッピングを保持）** — dynamic workflow の各 agent への model 割り当ての確定ルール。**fan-out agent は model を必ず明示**する（既定継承はコスト事故 — 新規セッション既定 `claude-fable-5` を継承すると実コスト 2 倍超）:
...
dynamic workflow の各 agent への model/effort 割り当ては **本書 §1.1「effort ルーティング」の「WF agent 段別 model/effort フルマッピング」が SSOT**（sc-41b で本書 §1.1 を SSOT に確定。global 配布物のモデル階層ルーティング節の縮退＝WF 外領分〔spawn worker の model 明示・fable=admin/consult main-loop 専用の例外・haiku 使い捨て〕のみへの反転と back-pointer 反転は連動タスクで実施予定で、現時点では global 節も WF-agent フルマッピングを保持している）。フルマッピング（dynamic WF は sonnet/opus のみ・sonnet=大規模探索/opus=review/verify 含む substantive・fable 不投入・`demoteFable`・`FABLE_MAX_CONCURRENCY`・un-8q5 改訂根拠）はそこを一次情報として読む。intra-doc 二重化を避けるためここに再掲しない。
...
| 本書 §1.1「effort ルーティング」 | WF agent 段別 model/effort 割り当ての**一次 SSOT**（sc-41b で確定・global 配布物側の縮退は連動タスクで実施予定・§4 はここへの内部 pointer） |
