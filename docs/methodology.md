...
| **リスク（risk）** | blast radius・不可逆性・outward 性（規約変更・全ホスト配布・外部公開） | **gate の厳しさ**（read-only gate・merge gate〔`protocol.md` §5.4 3 クラス判定〕の **AI gate 強度**〔人間確認は 3 クラス該当時のみ〕・refute 多数決の閾値・completeness critic） |

> **リスク軸の読み方（orch-vhiu 2026-07-25 裁定）**: 規約変更・全ホスト配布は**人間承認トリガーからは外れた**（§5.4 で廃止）が、**AI gate 強化トリガーとしては残る**——リスクが高い案件は「人間に聞く」ではなく「**AI 敵対 gate を厚くする**」で受ける（人間確認は 3 クラス〔消す / 出す / 使う〕該当時のみ・分類の正本は `protocol.md` §5.4）。
...
- **死んだ WF の一次診断は journal**: `journalctl --user --since "<時刻>" | grep -iE 'oom-kill|OOM killer'`（**2 系統の文言は排他**＝行数は実測。`grep -i oom-kill` だけだと KILLER 行を全数落とす。落ちた行が独立 incident かは**未確定**なので併記形で拾う。理由の SSOT = `protocol.md` §6）。`dmesg` / `/var/crash` / 起動時 stderr は**原理的に空振り**する（理由と手順の SSOT = `protocol.md` §6「CC プロセス突然死（cgroup OOM）の oracle」）。
...
**selftest grep の自己ヒット（`--` 後置で `--include` が無効化・bd un-wlk8）**: selftest の grep を `grep -rF -- PAT --include=GLOB DIR` と書くと、`--`（オプション終端）以降が**全て非オプション扱い**になり `--include=GLOB` が検索対象のファイル名へ化けて filter が効かない——grep は `--include=*.md: No such file or directory` を stderr へ出しつつ（exit 2）、残る DIR は **filter なしで再帰検索**するため、selftest script 自身や無関係ファイルへ自己ヒットして INFO/FAIL 誤報を出す（un-w11g cell で INFO 2 件誤報の実証・GNU grep 3.11 / ugrep の両系で再現確認済）。正形は **`--include` を `--` より前に置き、パターンは `-e` で渡す**: `grep -rF --include=GLOB -e PAT -- DIR`（`-` 始まりのパターンは `-e` が守るため、`--` はパス側の保護にだけ使う）。doc cell selftest を使う全 project 共通の罠（selftest 横断規約の SSOT は scribe 側 doc＝本項）。
