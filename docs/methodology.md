...
arg 露出は全段でなく個別 knob 2 つ（`reviewEffort` / `verifyEffort`）に絞る。**本表は決定済みの方針であって現行実装ではない**——実装は別 cell（sc-94z）で、それが着地するまで `cell-quality.workflow.js` は全 `agent()` へ `args.effort` を**一律**に適用する（`reviewEffort`/`verifyEffort` knob も guard 段の独立も未実装）。しかも worker 自己点検経路では `scripts/scribe-selftest-args.sh`（sc-7ac）が worker の実効 effort（`CLAUDE_CODE_EFFORT_LEVEL`・scribe-spawn が worker env-file へ注入）を allowlist 一致時に読んで cell-quality の `args.effort` へ**自動で焼く**（verified・コード read）——**標準 worker 経路では worker effort が WF の `args.effort` へ自動伝播する**。ゆえに medium で spawn した worker は自己点検 WF の guard 段（Review/Verify/Fix）まで medium へ一律降格し、但し書き(1)＝gate 側を下げないに抵触する。**運用帰結: sc-94z 着地までは worker spawn の effort を medium へ落とさない**——spawn 判定（`protocol.md` §1）で②が medium 適格でも high を選び、判定 1 行記録に「sc-94z pending ゆえ high」を理由として記す。
