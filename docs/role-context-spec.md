...
- gate funnel 手順（§5）: worker 報告監査 → cell-quality gate review（read-only・worktree 指定）→ findings 直読 → merge gate（§5.4 3 クラス判定・merge 自体は非トリガー）→ squash / 条件付き auto-merge → go-live → cleanup。
...
**禁止**: 特になし（admin は full 権限）。ただし **merge gate（§5.4 3 クラス判定・orch-vhiu 2026-07-25 裁定）** は admin の義務として注入する——**merge 自体は非トリガー**で、**ユーザー確認を取るのは 3 クラス（消す / 出す / 使う）該当と snapshot-mismatch トリガのみ**。それ以外（規約ファイル・全ホスト配布物・事前合意逸脱を含む）は **AI 敵対 gate 通過をもって AI 判断で merge** する（人間承認を外しても gate は外さない＝gate 必須化と対）。**分類の正本は `docs/protocol.md` §5.4**（本節は本文を転記せずポインタに留める）。無確認 auto-merge 時は対象 bd notes に証跡（`auto-merged: …`）を残す。
...
- **モデル規約**: 既定 **`opus[1m]`**（2026-07-25 ユーザー裁定＝bd `sc-pegi`）。alias 形（版 bump で pin が腐らない）+ 1M context（長寿命 main-loop の context 肥大に耐える）を買う選択。`--model` 明示は既定より常に優先で、`--model claude-fable-5` の明示も**受理される**（fable は既定から外れただけで締め出していない）。model 可否の実測 preflight と降格 fallback は**機構ごと持たない**（既定が opus 系のとき可否判定は死にコードで、残すと既定値から素 opus への silent 降格経路になり、この既定が買っている 1M context を失う）。consult は admin と同じ main-loop 系統ゆえ fable の明示起動が許される唯一の役割（WF agent への fable 投入とは無関係＝methodology §4。worker は fable 厳禁＝不変）。
  - **改訂系譜（裁定の履歴を消さない）**:
    - 初代: 「基本 opus・ユーザー指定時のみ fable」（consult 起動テンプレを外部 doc から本節へ移設した時点の規約）。
    - `sc-9q6`（2026-07-03 ユーザー確定）: 既定を fable（`claude-fable-5`）へ改訂し、実起動時 preflight（最小 `-p` 呼び出し・timeout 15s・timeout=受理／fast fail のみ不可）で fable が使えないときだけ opus へ loud fallback する機構を併設した。判定境界の根拠になった実測知見（正常な fable でも応答に 60s+ かかる一方、利用不可は ~5s で fast fail する）の原典は bd `sc-9q6` notes。
    - `sc-pegi`（2026-07-25 ユーザー裁定・**本裁定が `sc-9q6` の既定 fable を上書きする**）: 既定を `opus[1m]` へ改訂し、preflight と loud fallback を機構ごと撤去した。fable は「既定から外す」だけで `--model` 明示経路は残す。
  - 起動は `scribe-spawn --consult`（内部で `cld-spawn --model <解決値>` を直接呼ぶ）。`/session:spawn` の NLU は `--model` を解析しないため、model 制御はこの経路が確実。
