...
- **モデル規約**: 既定 **fable**（`claude-fable-5`・sc-9q6 2026-07-03 ユーザー確定）。`scribe-spawn --consult` が実起動時に preflight（最小 `-p` 呼び出し・timeout 15s・timeout=受理＝利用可/fast fail のみ不可）し、**fable 利用不可のときだけ opus へ loud fallback** する（silent 降格しない・dry-run は API を叩かない）。`--model` 明示は既定より常に優先。consult は admin と同じ main-loop 系統ゆえ fable が許される唯一の役割（WF agent への fable 投入とは無関係＝methodology §4。worker は fable 厳禁＝不変）。
  - 起動は `scribe-spawn --consult`（内部で `cld-spawn --model <解決値>` を直接呼ぶ）。`/session:spawn` の NLU は `--model` を解析しないため、model 制御はこの経路が確実（旧規約「基本 opus・ユーザー指定時のみ fable」は sc-9q6 で改訂）。
