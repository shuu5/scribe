...
- **どの unit が殺されたかを読む＝層の弁別**。`run-<id>.scope` なら per-scope 上限に当たったので下の `set-property` が効く。だが **scope の上位には user slice が居る**（`/user.slice/user-<uid>.slice/user@<uid>.service/app.slice/run-<id>.scope`）。oom-kill 行が **slice / `user@<uid>.service` 側**を名指しているなら、**per-scope 上限を上げても無駄**（上位が binding なら子の上限を緩めても天井は動かない＝**deduced**・cgroup v2 の階層制約に依拠する主張で本 fleet では実挙動未確認）。上位の実効天井は `cat /sys/fs/cgroup/user.slice/user-$(id -u).slice/memory.{max,high,current,peak}` で直読する（observable・2026-07-27 ipatho-server-2 実測: MemTotal 125GiB に対し **max 110GiB / high 100GiB** が設定済で、**peak が既に high へ到達**していた）。**`memory.high` は kill でなく throttle**（reclaim 圧で遅くなるだけ・**deduced**＝cgroup v2 の機構に依拠。本 fleet の worker sandbox では systemd/D-Bus へ触れず実挙動を確認できない）なので、**oom-kill 行が出ないまま session が異様に遅い**ときは high 到達を疑う——これは「窓は生きたまま idle」に見えるため **zombie 系と誤診しやすい**（分岐の第 3 枝）。
  ...
  **どの層が殺したかは永続カウンタで決まる（journal が trim されても残る）**: `memory.events`（**階層版**＝自層+子孫）と `memory.events.local`（**自層のみ**）を並べて読む。**差分が下位由来**を意味する。observable・同 host 実測: slice の `memory.events` = `high 5302 / max 97189 / oom 34 / oom_kill 4` に対し `memory.events.local` = `high 5302 / max 0 / oom 0 / oom_kill 0` ＝ **slice 自身は自層の max に一度も当たっておらず、oom_kill 4 はすべて下位 scope 由来**（＝この host では per-scope 上限を上げる対処が正しい）。一方 `high` は local にも 5302 立っており **slice 自身の throttle は常態的に起きている**。「いま throttle されているか」は `memory.pressure`（PSI）の `some/full avg10` を見る（`peak ≧ high` は「過去に一度でも触れた」しか言わない）。
...
**証拠種別（本 §6 凡例）**: oom-kill 行・時刻一致・`memory.max` / `memory.peak` / `memory.events{,.local}` の実測はいずれも **observable（admin / worker 検証済み）**。本節の**手順**は observable のみに立脚する。ただし手順の**解釈**に 2 点だけ mechanism 由来（**deduced**）の主張が混じるので、その場で明示してある——① 上位 cgroup が binding なら子の上限を緩めても天井は動かない、② `memory.high` は kill でなく throttle。どちらも本 fleet の worker sandbox（systemd/D-Bus 不可）では実挙動を確認できていない。**mechanism を verified 扱いで焼かない**（この規律違反が本節の過去の誤り＝「cgroup OOM は kernel log に出ない」を生んだ）。
