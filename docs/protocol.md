...
- **どの unit が殺されたかを読む＝層の弁別**。`run-<id>.scope` なら per-scope 上限に当たったので下の `set-property` が効く。だが **scope の上位には user slice が居る**（`/user.slice/user-<uid>.slice/user@<uid>.service/app.slice/run-<id>.scope`）。oom-kill 行が **slice / `user@<uid>.service` 側**を名指しているなら、**per-scope 上限を上げても無駄**（上位が binding なら子の上限を緩めても天井は動かない＝**deduced**・cgroup v2 の階層制約に依拠する主張で本 fleet では実挙動未確認）。上位の実効天井は `cat /sys/fs/cgroup/user.slice/user-$(id -u).slice/memory.{max,high,current,peak}` で直読する（observable・2026-07-27 ipatho-server-2 実測: MemTotal 125GiB に対し **max 110GiB / high 100GiB** が設定済で、**peak が既に high へ到達**していた）。**`memory.high` は kill でなく throttle**（reclaim 圧で遅くなるだけ・**deduced**＝cgroup v2 の機構に依拠。本 fleet の worker sandbox では systemd/D-Bus へ触れず実挙動を確認できない）なので、**oom-kill 行が出ないまま session が異様に遅い**ときは high 到達を疑う——これは「窓は生きたまま idle」に見えるため **zombie 系と誤診しやすい**（分岐の第 3 枝）。

  **どの層の上限が破られたかは永続カウンタで決まる（journal が trim されても残る）**: `memory.events`（**階層版**＝自層+子孫）と `memory.events.local`（**自層のみ**）を並べ、**`.local` の `max` / `oom` を主語に読む**——これらは「**その層自身の上限が破られた回数**」で計上される。**`oom_kill` を層の弁別に使ってはならない**: `oom_kill` だけは「**殺されたプロセスが所属していた層**」で計上されるため、**直接プロセスを持たない中間層では `.local` が構造的に常に 0** になる（observable・2026-07-27 実測: 当該 slice の `cgroup.procs` は **0 行**・`nr_descendants` 43 ＝ 比較の情報量がゼロ）。同 host 実測: 階層版 `high 5302 / max 97189 / oom 34 / oom_kill 4` に対し `.local` は `high 5302 / max 0 / oom 0 / oom_kill 0`。**同じ `.local` で `high` だけ階層版と同値**という非対称が、この計上主体の差の直接証拠になる。読み方は `.local` の `max 0 / oom 0` ＝ **この層自身の上限は破られていない**（＝下位の上限で決着している）／`.local` の `high 5302` ＝ **この層自身の `memory.high` は常態的に破られている**（throttle）。「いま throttle されているか」は `memory.pressure`（PSI）の `some/full avg10` を見る（`peak ≧ high` は「過去に一度でも触れた」しか言わない）。
...
**証拠種別（本 §6 凡例）**: oom-kill 行・時刻一致・`memory.max` / `memory.peak` / `memory.events{,.local}` の実測はいずれも **observable（admin / worker 検証済み）**。本節の**手順**は observable のみに立脚する。ただし手順の**解釈**には mechanism 由来（**deduced**）の主張が混じるので、その場で明示してある——① 上位 cgroup が binding なら子の上限を緩めても天井は動かない、② `memory.high` は kill でなく throttle、③ `memory.events{,.local}` の各フィールドの**計上主体**（`max`/`oom`/`high` は「その層自身の上限が破られた回数」だが `oom_kill` だけは「殺されたプロセスの所属層」）。いずれも cgroup v2 の ABI 定義に依拠し、本 fleet の worker sandbox（systemd/D-Bus 不可）では実挙動を確認できていない（③ は「`cgroup.procs` 0 行の層で `.local` の `oom_kill` が 0」という observable と整合するが、整合は機構の証明ではない）。**この列挙は閉じていない**——新しい解釈を足すときは同じ扱いで種別を付けること。**mechanism を verified 扱いで焼かない**（この規律違反が本節の過去の誤り＝「cgroup OOM は kernel log に出ない」を生んだ）。
