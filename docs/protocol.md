...
[起票・依存 wire(admin)] → spawn(§1) → worker 実装(§2 prompt 規約に従う)
   ...
   → findings 直読 → merge gate(§5.4 3 クラス判定・merge 自体は非トリガー) → squash/auto-merge → go-live → admin が close → cleanup) → dolt push 同期点(§3)
...
- **台帳境界**: write・所有するのは自 project（`sc-`）の台帳だけ。foreign 台帳（`un-` / `orch-` 等）は **read に留め write しない**。cross-repo 依頼は自 bead に `needs-orch` ラベル + 必須 3 項目を front-load する（prose deferral 禁止・§8）。courier 便の受信は**受け bead + 便 id 明記 + 初回整合返信**で会計し、orch への返信は open bead に置く（§8）。
...
- **selftest で pipefail 下の大出力 `producer | grep -q` を書かない（SIGPIPE 偽 RED・sc-65hg / courier orch-lcdu）**: `set -o pipefail` 有効な selftest 内で「大出力／無界／slow producer | grep -q PAT」を書くと、`grep -q` が最初のマッチで早期 exit した瞬間に producer が **`SIGPIPE`** で死に、pipefail が pipeline rc を **141** へ昇格させ、**アサーション充足でも偽 RED**になる（入力サイズ依存 flaky＝31KB body で 5 回中 3 回 rc=141・herestring 形は 200KB で 20/20 決定論 rc=0 を実測）。正形は producer 出力を先に確定させる **herestring**（`grep -q PAT <<< "$out"`）か **file-arg grep**（`grep -q PAT "$file"`）を使う。★`grep -c`／`grep -n`（count／行番号意味）は herestring が付ける末尾改行で数がずれるため **機械変換しない**（file-arg 化するか据置）。本節が規約本文 SSOT で、spawn prompt の test-first bullet（`scribe-spawn.sh build_prompt`）と `methodology.md` §2 selftest 落とし穴群がこの文言を引く（一次出典 pointer・平仄・drift 停止）。この footgun は既存の「selftest grep の自己ヒット（`--` 後置）」（methodology §2）と同じ selftest grep 系の別 footgun。
...
- **WF / subagent 起動主張の知覚健全性（runId 突合・transcript 実在 oracle・un-y3i5 第 3 変種 / sc-5cbx・bd sc-ugb8）**: worker は Workflow を起動したら**起動直後に直前の tool result の runId を突合してから**待機に入る。run id・agent 数・phase 数を報告文へ書く**前に**、直前の tool result と照合する（confabulation の自己防止＝起動や runId を「書きながら創作する」経路を塞ぐ）。背景 subagent（Agent tool）を spawn したときの**成功 oracle は自分の主張でなく transcript の実在**——`<config-dir>/projects/<encoded cwd>/<sid>/subagents/` 配下の `agent-*.jsonl` を ls で確認してから通知を待つ。実証 = bd sc-5cbx 初代（『portC を起動した』主張に Agent tool_use が 0 件＝admin が tool_use 全数列挙で幻影確定）・bd un-n6jy 初代（cell-quality WF の起動と runId を捏造し永久待機・差戻しにも別の捏造 runId で応答）。一次出典: bd un-y3i5（uns relay・courier orch-at46）・doobidoo `923c4c30` / `85f8b5b4`。検知側（admin probe）= §5 step1「WF 起動主張の実在照合」・回復側 = §6「幻影 lore」の respawn 一択。carrier 注記: 本項が本文 SSOT で、`scribe-spawn build_prompt` への operative 焼込 + reach pin は **bd sc-42l7 が追跡**（焼込前は spawn worker へ届かない＝doc literal のみの reach 未達を既知として明示）。
...
- **background fan-out 禁止（sc-f6sw）**: worker prompt は「検証 suite 等を background shell で多数並走させる fan-out の禁止 / 完了待ちは単一の blocking 実行 / 並列は WF 骨格（Workflow tool）へ委譲＝正規経路（cell-quality WF の直接呼出は対象外）」を 1 条項として焼く。carrier は scripts/scribe-spawn.sh の build_prompt。
...
- **findings の差し戻しは autoFix 後の HEAD で再確認してから（`blocking[]` を額面で読まない・sc-0mas / 実害 sc-zwzs 2026-08-04）**: cell-quality 等の WF 返り値の `blocking[]` は **autoFix が走る前の round の findings** を保持する。errata として worker へ差し戻す前に、(1) run record の `logs`（`round N: autoFix 適用 M 件 (self-test PASS, amended)` 形。locator = §5「gate の義務」の run record 補記）で autoFix 適用の有無を**機械的に**読み、(2) **autoFix 後の HEAD の実体**（`git show <実装commit>:<file>` / 実走）で「まだ欠けているか」を確認する。(3) `ESCALATE` かつ `machineryFailed` の run では最終 round の `blocking=0` も信頼しない代わりに、**過去 round の blocking も現 HEAD で再検証してから**扱う（両方向の誤りを塞ぐ）。理由（load-bearing）= `blocking[]` 単独で差し戻すと **land 済み修正の二重実装**を worker に強いる（実例 = admin が `blocking[]` を salvage note へ「残作業」として転記し、worker の独立検証で訂正された。一次証拠 = run record の logs に autoFix 適用行が実在し、当該 assert が実装 commit の blob に実在した）。
- **escalate を追加レビューのトリガにしない（トリガは unreviewed post-WF delta の有無・sc-i2kc 追加1・doobidoo `ff25f370`）**: 「cell-quality が escalate で返ったら gate 側で追加レビュー」は粗すぎる。トリガは **「WF 審査を通っていない差分（WF 完了後の手修正 commit）の有無」**に条件づける——① escalate の原因が品質か機械故障（machineryFailed・account 上限による reviewer null 死）かを弁別 → ② `git log` で WF 完了後の commit を機械確認 → ③ 在れば敵対レビュー必須・無ければ機械実測で受理してよい（★前提: 敵対レビュー round が 1 度も完了していない run では**全 diff が unreviewed delta**＝③ の受理経路に入らず必ず再走する——post-WF commit 0 は「審査済み」を含意しない・sc-i2kc gate B2） → ④ 受理時は**なぜ再走しないかを close reason に明記**する。根拠 = 同日 3 cell の実測対比: 実害 2 件は escalate 原因が品質**かつ** post-WF 手修正 commit が存在・1 件は機械故障かつ respawn 後の新規 commit ゼロ（unreviewed delta が構造的に不在）で full 再走は費用対効果ゼロ。
- **go-live 検証は merge 前に「変更集合が空」の文脈でも回す（teeth の feature-branch 文脈依存・sc-0mas 併記 / 実害 sc-3kfn）**: scope-assert の vacuity 防止（`[ -n "$changed" ]` 等）のように **merge-base と HEAD の差分に依存する teeth** は、squash merge 後の main では merge-base==HEAD となり**構造的に常時 RED**（baseline 汚染）になりうる。go-live 検証を merge 後に回すと land 後にしか検出できないため、**merge 前に隔離 clone（`git clone --shared`）で「merge 後 main 相当＝変更集合が空」の文脈でも teeth を回す**。理由（load-bearing）= 検出時点を merge の前へ動かすだけで、baseline を汚す land を構造的に防げる（実例 = go-live で scope assert が main 常時 RED 化し、修正が別 bead へ繰り越した）。
...
- **WF 外側ループ 4 点（再走可否の機械判定・resume 再走・escalation 計数・値札記録）は機械実装が SSOT（sc-46kv・M0 外側 4 裁定）**: `scripts/scribe-wf-rerun-judge.sh` と `scripts/scribe-wf-cost.sh` の **usage header が規約本文**（判定の rc 契約・記帳 marker と値札の文法・resume 手順と効かない 3 条件・自動再走 1 回まで、を含む——本節へ機械語彙を verbatim 複製しない＝上の CALIB bullet と同じ drift 防止）。数値閾値・severity 集合は env/args が SSOT（P1 実分布待ち・doc に決め打ちしない）。
...
- **(b) selfTestCmd を gate が自ら再実行する義務**: gate は actor の「self-test PASS」報告を信じず、契約 bead の verification 欄（`検証:`）の **selfTestCmd を自ら再実行**して ground-truth を取る。actor 報告の丸ごと捏造（`orch-wzq`）は self-report の机上監査では捕捉できず、gate の独立再実行のみが捕捉する。selfTestCmd は bead notes の **verification 欄の value から取得**する（欄の位置規則は下記 snapshot 節と共通）。再実行が FAIL した／実行できない場合は merge せず errata（§4）or salvage（§6）へ回す。★**直列化（sc-5cwp 実測・偽 RED の構造回避）**: gate の selfTestCmd 独立再実行と admin の SUT 実走（bats 等）を**同一 worktree で並走させない**——並走 race は transient FAIL を生み、gate が『worker の虚偽報告』と誤裁定して errata / salvage へ誤送りする（実測 = probe exit=1 → 直列再実行で rc=0×2 の決定論＝偽 RED 確定・bd sc-5cwp notes [GATE incident 記録]）。probe が非 0 を返したら差し戻す前に必ず admin 自身が**直列で 2 回**再実行し決定論を確認する。rc 捕捉を `cmd | tail` 越しにしない（`$?` が tail の rc になる）。
- **(c) acceptance snapshot mismatch = auto-merge 資格剥奪 → 人間 ratify 昇格**: dispatch 入口で bead notes に焼いた acceptance snapshot（下記形式）と、gate 時点の現 acceptance が **mismatch**（契約が dispatch 後に書き換わった）していたら、その cell は §5.4 の auto-merge 資格を**剥奪**し、機械判定に関係なく**人間 ratify へ昇格**する（fail-closed）。契約すり替えを merge が素通しするのを防ぐ（§5.4 の snapshot-mismatch トリガと対応）。
...
- **(補) WF run record の locator（§4「findings の差し戻し」が参照する一次面・sc-aprn R2）**: WF の run record は `<config-dir>/projects/<encoded cwd>/<session-uuid>/workflows/wf_<runId>.json`——**完走時のみ生成**され（実行中・死亡 run には無い）、**session-uuid 階層が必須**（`projects/<encoded>/workflows/` 直下という浅い path は実在しない＝gate verify が直下 0 件を実測済み）。(d) transcript と同じく **config-dir 相対**で解決する（worker を起こしたアカウントの config dir・`~/.claude` 決め打ち禁止）。journal.jsonl は subagent イベント列で帰属面ではない＝単価・帰属の一次面は run record（workflowName + logs + workflowProgress）。
...
1. **worker 報告の監査**: worker の self-report（commit hash / WF 返り値 JSON / `receivedArgs` / self-test 結果 / 成文化・実装した内容）を読み、args 解決の成否（`parseFailed`/`receivedArgs`）と self-test の pass を一次確認する。
   ...
   - **WF 起動主張の実在照合（un-y3i5 第 3 変種の検知 probe・courier orch-at46 / bd sc-ugb8）**: worker が「WF を起動した／runId=wf_…」を報告したら額面で信じず、worker transcript の tool_use 集計（「gate の義務」(d) transcript-forensics と同法）に Workflow tool_use が実在するか + `<config-dir>/projects/<encoded cwd>/<sid>/subagents/workflows/wf_*` dir の実在を突合する（両方不在 = 起動捏造・confabulation 第 3 変種）。差戻しへの応答も別の捏造 runId で返る型ゆえ、確定したら §6「幻影 lore」の **respawn 一択** playbook へ回す（2 段差戻しは効かない）。一次出典: bd un-n6jy notes [ADMIN-CORRECTION--un-n6jy]/[ADMIN-RESPAWN--un-n6jy]・bd un-y3i5（uns relay）・doobidoo `923c4c30`。
   - **worker 申告の bats not-ok は sandbox 環境汚染でありうる（「退行ゼロ」の根拠に使えない・sc-von0 gate-4 実測）**: worker の bats 実行は既定で OS sandbox 内（§1 sandbox 節）＝環境要因の not ok が混入する（実測 = worker 申告 1464 ok / 114 not ok に対し admin 実測 1578 ok / 0 not ok / rc=0・母集団 1464+114=1578 の**完全一致**＝114 件全てが sandbox 由来で repo の状態ではない）。**退行ゼロ判定は admin の sandbox 外実測を truth** とし、worker 申告の not ok 数を単独で「repo の退行」と読まない（母集団一致の確認で環境要因と切り分ける）。
...
3. **findings 直読（refuted も鵜呑みにしない）**: WF の返り値 findings を admin が直読検証する。WF 内の adversarial verify が `refuted` と判定した finding も鵜呑みにせず、admin が一次監査する（gate は薄い一次監査・WF 返り値の机上承認ではない）。スコープ外（他リポ・他 cell の領分）を求める finding は却下し、却下理由を記録する。**差し戻す（errata を出す）前には、`blocking[]` を額面で読まず autoFix 後の HEAD の実体で再確認する**——手順は §4 の「findings の差し戻しは autoFix 後の HEAD で再確認してから」に従う（`blocking[]` は autoFix 前 round の findings を保持するため、単独で差し戻すと land 済み修正の二重実装を強いる）。
4. **merge gate（3 クラス判定・merge 自体は非トリガー）**（orchestrator grill orch-8is・2026-06-26 ユーザー ratify → orch-vhiu 2026-07-25 で 3 クラスへ改訂）: **merge という行為そのものは人間確認の必須トリガーから外す**。ユーザーは自分でコードを読まないため「全 merge を最後に確認する」のは情報量ゼロで意味がなく、実質の安全弁は下記の **AI gate（findings 直読＝step3）と 3 クラスの機械判定**に置く。確認を取るのは **下記 canonical block の 3 クラスに該当する変更**と **snapshot-mismatch トリガ**（orch-tdj で scribe 側に追加した独立 fail-closed トリガ・orch ①-④ 番号を持たない）**だけ**で、それ以外は admin が確認なしで auto-merge する（step6）。**旧②③（規約ファイル / 全ホスト配布物）の human ratify は廃止**し、**同時にそれらの変更は AI 敵対 gate（step2 の cell-quality review + step3 の findings 直読）を必ず通す gate 必須化へ置き換える**（人間承認を外しても gate は外さない＝この 2 つは対で成立する）:
   ...
   - **註（§5 step5「merge 前の作法」との接続・sc-i2kc gate E4）**: branch **未 push** の `git reset --soft <base>` + 単一 commit への畳み（**tree 差分が空**の lossless 証明付き）は **(a) 消す に当たらない**——履歴はまだ公開面に存在せず（除去対象の露出が未発生）、機械 guard 層（git-destructive-guard）の deny 対象（`reset --hard`）でもない。本件（未 push soft-reset）の該当性解釈は本註が SSOT（§5 step5 側は pointer のみ持つ・canonical block 本文には触れず、orch-vhiu 裁定の枠内の整理として新権限を作らない）。
   ...
   - **設計の分岐は grill 提案として上げる（承認要求ではない）**: 複数の妥当な設計が併存し、選択が人間の目的・価値観に依存するときは、承認を求めるのでなく **grill 提案**（§7）として上げる。事実で決まるなら止めない。順序・選択肢の是認だけを求める問いは出さない（AI が推奨を出し、決めて進む）。旧「設計ソフトズレ」（AI gate が高確信した逸脱でユーザー確認を取る）は承認トリガーとしては廃止され、gate 強化と本経路へ移行した。
   - **snapshot-mismatch トリガ（機械 fail-closed・§5「gate の義務」(c)）**: dispatch 入口で焼いた acceptance snapshot（§5「acceptance snapshot の形式・照合手順」）と gate 時点の現 acceptance が **mismatch** した cell は、3 クラス該当/非該当に関係なく **auto-merge 資格を剥奪し人間 ratify へ昇格**する（契約が dispatch 後にすり替わった＝合意基盤が動いたため fail-closed）。照合は最新 header の `sha256=` と現 acceptance の再計算（zero-normalization）の比較のみで行う（§5 snapshot 節）。**本トリガは 3 クラスのいずれにも属さない tamper 検知**ゆえ 2026-07-25 裁定でも改廃せず据え置く（orch-dispatch 入口 gate との byte 整合が SSOT で、scribe 単独改訂は cross-repo 契約を壊すため）。
   - **適用範囲**: 本 3 クラス判定は self-dev に限定せず、**foreign（他 project の cell）を含めて広く**適用する（cross-ledger の push 経路も §5 origin verify と同じ gate chain を通る・§8）。
   - **事後証跡（auto-merge 時の必須記録）**: **3 クラス非該当かつ snapshot-mismatch トリガ非該当**で admin が確認なしで auto-merge した場合は、対象 issue の `--append-notes` に **`auto-merged: <gate 判定要約> + <PR/commit>`** を必ず残す（無確認 merge を後から追跡可能にする＝§4 の gate findings notes-append と同じく review trail を永続化する）。
   - **A. park の発火条件（可変・トリガー定義の側）**: **3 クラス該当**または **snapshot-mismatch トリガ該当**で cell を人間 ratify へ昇格し PR 作成後 park に入るときに、下記 B の機械可視化を必ず行う。
   - **B. ratify 待ち park の機械可視化（必須・不変）**: **本様式はトリガーの定義に依存しない**（発火条件が縮小しても様式は存続する）。park に入る時点では、対象 bead へ **`needs-user` ラベルを付与**し、**notes へ `GATE-SUMMARY(...)` 一行**（**どの park トリガに該当し何が残るか**を書く。テンプレを green 固定にしない——3 クラス該当は「3 クラス touch・AI gate green・残 = human merge ratify のみ」、snapshot-mismatch は「acceptance snapshot mismatch の要約・human 裁定待ち」。gate が問題を検出した park では green と書かない＝§7.2 可視性要件 (c) で notes 先頭を読む human を rubber-stamp merge へ誘導しない）を `--append-notes` する。🔴 バナー（pane 表示・§7.2 可視性要件）は人間向けで **orchestrator の滞留 scan（ラベル/notes ベース）には乗らない**——ラベル + notes-append（`updated_at` を前進させる・§8 bead-append 規律と同型）が park の機械宣言となり、これを欠くと park 中の cell が「idle × 未終端」と誤検知される。ratify → admin close でラベルは scan 対象外となり自然消滅する。一次出典: orch relay 2026-07-23（sc-qqms が 1h 滞留扱いになった誤検知の実測）・bd sc-8a6p・運用実績 = sc-qqms / sc-kxec / sc-9954 / sc-1d95（`verified`）。
5. **push 前に origin URL 健全性を verify（汚染なら fail-loud + 復元）**（bd un-1n1）: squash merge / GitHub push の **前に** リポの `origin` URL が spawn 時 canonical のままかを照合する。worktree は anchor と `.git/config`（remotes）を共有するため、worker が origin を mutate すると anchor+全 worktree の origin が壊れ push が破綻する（2026-06-16 un-v5x 実害＝134s タイムアウト+gh が known host 無しと誤認）。汚染を検知したら **fail-loud し、canonical へ復元してから** push する（汚染したまま push しない）。
   - **中間 commit message の leak は push した瞬間に確定する（merge 前の作法・sc-i2kc 追加3・doobidoo `ff25f370`）**: commit message に leak（実名 / 絶対パス / private 値）が残ったまま push すると GitHub の PR commit 一覧に載り**取り消せない**（3 クラス (b) 出す の一方向性）。branch **未 push** の段階なら `git reset --soft <base>` + 単一 commit へ畳むことで露出ゼロで除去できる——**tree 差分が空**であることを確認して lossless を証明する（3 クラス該当性の判定は §5.4 の註が SSOT）。worker に history rewrite を強いない。
   - **leak battery の unresolved-hex 系統（名前ベースでは原理的に捕まらない型・実装 = bd sc-pion）**: 名前ベースの既存系統（realname / shortname / deploypath——engine 実装はこれに dangling-lib を加えた 4 系統・sc-pion ■14）は「名前」を見る——**commit id 等の不透明 hex token は名前検査で捕まらない**。added-line の hex token を `git cat-file -t` に通し**本 repo で解決しないものを検知する** 1 コマンドの機械判定（検知は入口であり最終判定ではない＝下記 ★ の手順が正）を **unresolved-hex 系統**として leak battery に含める（**序数呼称〔第 N 系統〕は使わない**＝sc-pion ■4 裁定・規律本文 = 本項が SSOT・実装所有 = bd sc-pion・一次記録 = doobidoo `ff25f370`）。★判定面の carve-out（本規律を含む codify commit 自身への dogfood で確定・sc-i2kc gate E1）: RED 対象は「**git object を指す文脈**の hex」であり、**同一行に非 git 出典語（doobidoo / relay / 便 id 等）を伴う token は対象外**——一次出典 pointer 義務（doobidoo hash の記載）と衝突させると codify commit が恒常 RED になり override 常態化＝実質 fail-open に反転する。それ以外の未解決 hex も即 RED でなく **loud 化**（「なぜ本 repo に無い hex か」の説明を要求し、説明が立たないものだけ RED）＝sc-pion の偽陽性処理と同方式。
...
6. **squash merge → go-live → close（条件付き auto-merge）**: **(1) auto-merge 条件（可変）** = step4 の merge gate 判定で **3 クラス非該当かつ snapshot-mismatch トリガ非該当なら admin が確認なしで auto-merge**（squash merge）し、go-live（配布・反映）する。**auto-merge した場合は step4 の事後証跡（`auto-merged: <判定要約> + <PR/commit>` を対象 issue notes へ append）を必ず残す**。**(2) 3 クラス該当時の確認（縮小）** = 3 クラス該当または snapshot-mismatch トリガ該当の cell は**ユーザー確認が取れてから** squash merge する（確認トリガーはこの 2 つに縮小され、規約ファイル / 全ホスト配布物 / 事前合意逸脱は含まれない）。
   - **(3) merge 後の close は admin 専有（不変）**: **merge 後、admin が当該 issue を `bd close`**（理由に PR / squash commit）——**worker は自己 merge も自己 close もしない**（§4 反転・gate 分離）ので close は admin の funnel に属する。
   - **go-live の teeth は merge 前に「変更集合が空」の文脈でも回す**（§4 の同名 bullet に従う・sc-aprn R1 wire）: merge-base 依存の teeth は squash merge 後の main で構造的に常時 RED 化しうるため、squash merge → go-live を踏む**この step の前**に隔離 clone（`git clone --shared`）で「merge 後 main 相当」文脈の teeth を回す——検出時点を merge の前へ動かすことが baseline 汚染 land の構造予防（手順・理由の本文は §4 が SSOT）。
...
- **N（並列 worker 数）の上限 = admin funnel 律速**: **gate 逐次 + errata 往復**が直列なので、funnel がボトルネック（ユーザー確認は 3 クラス該当時のみ＝常時の直列要因ではない・2026-07-25 裁定）。pilot 実測で **N=3 が快適**、N=4-5 はタスク所要が異質な編成なら成立（実測値は un-8q5 pilot 由来で据置——新しい上限を主張するには新たな実測が要る）。
...
> 一次出典: doobidoo `3b838167`(Wave2)・`ac9022d8`(Wave1)・un-8q5 pilot（funnel 純 review 10-13min/PR・N 上限 = funnel 律速・snapshot 合成で diff 静的供給不要・adversarial gate 検出力／`verified`）/ bd un-1n1（origin URL 健全性 verify = un-v5x funnel 実害の対策③・push 前 verify+復元を規約化）/ **orchestrator grill `orch-8is`（2026-06-26 ユーザー ratify: step4 を merge 非トリガー化し 2 分類の確認トリガーを置いた回・auto-merge 時の bd notes 証跡。決定 SSOT = orch-8is notes / scribe 側追跡 = bd sc-97f / orchestrator 側追跡 = orch-an2）※本 2 分類は `orch-vhiu`（2026-07-25）により **supersede** — 旧①事前合意逸脱・旧②規約ファイル・旧③全ホスト配布物は人間確認トリガーから廃止（AI 敵対 gate 必須化と対）、旧④新規 outward は 3 クラスの「出す」へ移行。merge 非トリガー化と証跡義務は存続** / **scriptorium `orch-vhiu`（user 裁定 2026-07-25・承認体制の自律化＝人間確認は 3 クラス〔消す / 出す / 使う〕のみ・canonical block の文面 SSOT）/ courier `orch-6aum` / 本反映 cell = bd `sc-o2nm`** / **sc-e93 / orch-tya（step8 の publish 鮮度 advisory lint = 案A・Q1-Q4 orchestrator 合意: Q1 信号=updated_at〔orch parity と同一信号〕 / Q2 provenance=bead notes `federate-published-at:` marker / Q3=warn非block〔§8 と両立する advisory 上限線〕 / Q4=案A〔orch parity notice を待たない早期 self-check・代替でなく補完で並走〕。実装 = `scripts/scribe-publish-freshness.sh` + `tests/publish-freshness.bats`）** / **orch-tdj / orch-vji / orch-wzq（「gate の 3 義務」= gate は acceptance を逐条 PASS/FAIL 判定し・actor 報告を信じず selfTestCmd を自ら再実行し・acceptance snapshot mismatch は auto-merge 資格剥奪→人間 ratify 昇格。ground-truth 検証が完遂総括の丸ごと捏造を捕捉した実証 orch-wzq が動機・snapshot 形式/照合は orch-dispatch 入口 gate〔orch-vji land 済〕と byte 整合。scribe 側追跡 = 第5便 RiderA）※この snapshot-mismatch トリガは 3 クラスのいずれにも属さない機械 tamper 検知ゆえ `orch-vhiu`（2026-07-25）でも **据置**（supersede されない・改廃しない）**。
...
**回復判断（差戻しへの応答が捏造なら respawn 一択・un-n6jy 第 3 変種・bd sc-ugb8 / sc-i2kc）**: worker の WF 起動・runId 捏造（検知側 = §5 step1「WF 起動主張の実在照合」）へ durable bd note + scribe-inject で差し戻しても、**別の捏造 runId で応答する**＝是正への応答自体がさらに深い捏造になる。この signature を確定したら**追い inject せず respawn 一択**（context 汚染ゆえ 2 段差戻しが効かない・作業は durable 面〔commit / bd notes〕から回収する）。同系列の 3 変種を弁別して扱う: **un-o6gy 変種**（wait-stance idle＝実在する WF の完了通知待ちで止まる→操舵で再開可）/ **un-xm47 変種**（偽陰性＝marker・claim・pane の 3 点が揃えば健全→誤 salvage しない）/ **un-n6jy 変種**（捏造＝tool_use 不在→respawn 一択）。一次出典: bd un-n6jy notes [ADMIN-CORRECTION--un-n6jy]/[ADMIN-RESPAWN--un-n6jy]・bd un-y3i5（uns relay・courier orch-at46）・doobidoo `923c4c30`。
...
- → **適用面は下記「stall watchdog」節を併読**（本 regex を pane の tail 数行にだけ当てると spinner 行を見落とす実誤りあり＝全 capture へ当てる）。
...
### stall watchdog（終端 monitor の構造盲点 = idle 停滞の能動検知・sc-i2kc / sc-rr8b）

終端 monitor（`gate-pending` ラベル / `STATUS: blocked` / zombie sentinel / window 消失）は『**idle のまま停滞**』を構造的に検知できない——状態が何も変わらないため（実害 = sc-5cbx で約 2 時間の silent stall を人間の指摘まで見逃した・本節追加の一次動機）。spawn 時に **stall watchdog を併走**させる:

- **発報条件**: busy regex 不一致 × commit / file 活動なし × 終端ラベルなし が**約 20 分継続**。既存の zombie signature（§6「監視の規律」= idle-at-prompt **数分**持続 × 0-commit → salvage トリガ）は**先に発火する fail-closed の主網**であり、本 watchdog は **commit が出ている cell も含む上位網**（20 分は「活動なし」全種に対する閾値）＝両者は矛盾でなく層。
- **busy 判定は tail 数行でなく全 capture に対して行う**: spinner 行は入力欄 box の**上**に出るため tail 切りは見落とす（実誤りあり）。`capture-pane` 全量に busy regex（上記「busy 判定の regex」）を当てる。
- **発報後は admin が transcript を弁別して操舵 or salvage**: 停滞は少なくとも 3 型——**planning 停滞**（操舵 inject で再開可）/ **幻影待ち**（実在しない WF / 通知を待つ＝§5 step1「WF 起動主張の実在照合」で確定し respawn へ）/ **空応答劣化**（§6 salvage へ）。
- **quota 枯渇は stall watchdog でしか検知できない（sc-i2kc 追加4・doobidoo `9893cfc0`）**: 5h / weekly 上限に当たった worker は pane にメッセージを出して turn が切れるが **bead status は in_progress のまま**（終端 signal ゼロ）。かつ **pane status bar の残量表示は上限到達時点で凍る**ため現在値として読まない（実測 = 凍結表示が 7d:100% を示す一方 usage コマンドは同 account を 21% と報告し別 cell は正常稼働）。判定は必ず **usage コマンドとの突合**で行う。対話 prompt 経由の枠切れが inline tool-result 形で出て旧 2 択 picker 非出現・表示後 idle へ戻る型（orch 実測 v2.1.223・relay 6116cdfb7cca）も本 watchdog の受け持ち。account 切替 respawn の移動手順 SSOT = doobidoo `9893cfc0`。
- **張り方は §6「worker 監視の正規経路」に従う**（harness の Monitor + read-only poll script 形・sc-i2kc gate B3）——同 §6「監視の規律」の『自前の監視 poll を作らない』と衝突しない＝時間 cap 付き自前 bash ループ・野良 background poll で本 watchdog を実装しない。

> 一次出典: bd sc-5cbx notes [ADMIN-STEERING-2] / [SALVAGE-RESPAWN]・bd sc-rr8b（silent stall 実害の移送元・sc-i2kc へ統合）・doobidoo `85f8b5b4`（横断 incident 教訓）・orch 側の pane frame 分類 lib（scriptorium 所有・bd sc-koox と相互参照）。
...
**本節の適用前提（3 つの例外を先に潰す）**: ① **systemd-run 不在 host では cld は上限なしで直接起動する**（起動時 stderr に `cld: メモリ上限なし` が出る＝この session には per-scope 上限も一次対処〔`set-property`〕も無い。**この場合に user journal へ何が出るかは未確認**なので、まず下の oracle を踏み、層は各層の `.local` 直読で決める）。② **`--transport bg` の native background agent は独立 scope を持たず親 session の scope 予算を共有する**（deduced: cgroup は fork 継承）＝bg worker の暴走は**親 admin scope の oom-kill として現れる**。③ **cld を経由しない起動（素の `claude` 直起動・IDE 統合・cron 等）は scope を持たない**——`cat /proc/self/cgroup` の末尾が `run-<id>.scope` でなければこの session に per-scope 上限は無く、一次対処（`set-property`）も当たらない。**この場合に user journal へ何が出るかは未確認**なので、まず下の oracle を踏み、層は各層の `.local` 直読で決める。**「cld で起動した＝上限が効いている」とも限らない**: `CLD_MEMORY_MAX` が env に居る session は自動導出（下限/上限クランプ）を**丸ごと迂回**しているので、まず起動時 1 行が `[CLD_MEMORY_MAX 明示]` か `[auto: MemTotal …]` かを見る（`env | grep CLD_MEMORY` でも可）。**明示値を fleet へ配るなら carrier は env-file**（`cld-spawn` が `--env-file` > `CLD_ENV_FILE` > `~/.cld-env` の順で source する）——呼出元 shell の `export` が spawn 先へ届くかは tmux server の環境に依存する（deduced・本 sandbox では socket を作れず未実測）ので、そこに依存した手順を書かないこと。

**症状（この形なら本節を疑う）**:

- **自 session（admin / 自分自身）**: background の `Workflow` / `Monitor` が **journal に result 0 のまま消える**（＝「失敗した」痕跡すら残らない）・session 自体は `-c` で復帰するが **WF は戻らない**・重い dynamic workflow の実行中に**繰り返し**起きる。
- **worker を見ている admin から**: **窓ごと終了表示で落ちる**（`=== claude exited with code <N> ===`・SIGKILL 由来）。これは上の degraded / zombie salvage の署名（**窓は生きたまま** pane idle-at-prompt 持続 × 0-commit）と**明確に別物**——分岐則は「**窓が生きたまま idle = zombie 系 / 窓ごと終了 = 本節**」。窓が消えた・終了表示で止まった worker は、pane 代筆回収 → respawn の zombie 手順へ流れる前に**本節の oracle（journal の oom-kill 行と当該 worker の最終書込時刻の照合）を先に踏む**こと。

**oracle（第一手・これ一つ）**:

```
journalctl --user --since "<消えた時刻>" | grep -iE 'oom-kill|OOM killer'
```

出る行は 2 系統ある — `<unit>: A process of this unit has been killed by the OOM killer.`（以下 **KILLER 行**）と `<unit>: Failed with result 'oom-kill'.`（以下 **oom-kill 行**）。**この 2 つが対で出るのは scope を名指す行のとき**で、slice を名指す行では KILLER 行しか出ない（実測は下記）。**行が名指す `<unit>` が何を意味するかは断定できていない**——下の「層の弁別」を必ず読むこと（層の判定に unit 名を使わない）。**死んだ WF の agent 最終書込時刻と行の時刻が一致すれば確定**（実測: 2026-07-26 の 00:45:32 / 00:51:18 は **KILLER 行**の時刻で、調査 WF 2 試行の最終書込と一致した）。

**oracle を読むときの 2 点（層と同定）**:

- **層の弁別に journal の unit 名を使わない（最重要・誤診断の罠）**。**名指された unit が何を指すかは、本 fleet の実測からは断定できていない**。観測できたのは次の 2 点だけである（observable・2026-07-27 本 host 実測）: ① KILLER 行の中に **`app.slice` と `-.slice`（root）を名指す行が実在する**。② その `app.slice` は **`cgroup.procs` 0 行**（直接の所属プロセスが無い）かつ **`memory.max` / `memory.high` とも `max`**（上限が設定されていない）で、`.local` も全 0。**この 2 点から言えるのは「名指された層は binding 層でも直接の所属層でもない」という否定形だけ**で、肯定形の意味づけ（例:「被害プロセスの所属層」）は観測が支持しない。→ **運用はこれで足りる**: **どの層で上限が破られたかは下の `.local` の `max` / `oom` の直読で決める**。階層は `cat /proc/self/cgroup`（`/user.slice/user-<uid>.slice/user@<uid>.service/app.slice/run-<id>.scope`）で辿り、**各層について `.local` を読む**——名指された 1 層だけを見て決めない。
  - **最小限の規則（規則 R1・deduced）**: 名指された unit は **その subtree で kill が観測された層**であり、**祖先も同時に名指されうる**（上の観測①②と整合するが機構は未確認）。**規則 R2・deduced**: 上位 cgroup が binding なら子の上限を緩めても天井は動かない（cgroup v2 の階層制約）。**R2 の「上位が binding か」の判定に unit 名を使ってはならない**（判定は `.local` 直読）。両規則とも節末の証拠種別凡例に登録してある。
  - **oracle の grep は 2 系統の文言を両方拾うこと**。実測（observable・2026-07-27 ipatho-server-2・`journalctl --user` 全期間）: **KILLER 行 8 / oom-kill 行 5 / 併記 grep 13 / 両方を含む行 0** ＝ **2 つの集合は完全に排他**（8+5=13）。unit の内訳は KILLER 側が `app.slice`×2・`-.slice`×1・`run-*.scope`×5、oom-kill 側は `run-*.scope`×5 のみ。したがって `grep -i oom-kill` が落とすのは **KILLER 行の全数**である。**ただし KILLER 行だけが出ている行が独立 incident かは確定していない**（本 host の該当は 3 行・incident としては 2 件）——2026-07-27T00:59:30 の `app.slice` / `-.slice` 2 行は同秒の `Reloading finished in 94 ms.` と同時刻で、現 boot（`uptime -s` = 2026-07-25 08:00:29）の scope 名指し KILLER 行は distinct 5 本、`app.slice/memory.events` の `oom_kill` も 5（`.local` は 0）。残る 2026-07-24 の 1 行は前 boot でカウンタが失われており**判定不能**。第一手は `grep -iE 'oom-kill|OOM killer'` にする。**このコマンドは inline コピーが 3 箇所ある**（本節 / `cc-session/scripts/cld` ヘッダ / `docs/methodology.md` §2 の一次診断 bullet）——**scribe は他プロジェクトへ配布される plugin なので、片側だけ直すと禁止形が全ホストへ配られる**。変更するときは 3 箇所すべてを同時に直すこと（`grep -rn "oom-kill" --include='*.md' --include=cld` で洗える）。上位の実効天井は `cat /sys/fs/cgroup/user.slice/user-$(id -u).slice/memory.{max,high,current,peak}` で直読する（observable・2026-07-27 ipatho-server-2 実測: MemTotal 125GiB に対し **max 110GiB / high 100GiB** が設定済で、**peak が既に high へ到達**していた）。**`memory.high` は kill でなく throttle**（reclaim 圧で遅くなるだけ・**deduced**＝cgroup v2 の機構に依拠。本 fleet の worker sandbox では systemd/D-Bus へ触れず実挙動を確認できない）なので、**oom-kill 行が出ないまま session が異様に遅い**ときは high 到達を疑う——これは「窓は生きたまま idle」に見えるため **zombie 系と誤診しやすい**（分岐の第 3 枝）。

  **層の判定は永続カウンタの直読で行う（journal が trim されても残る）**: `memory.events`（階層版）と `memory.events.local`（`.local`）を**並べて読む**。同 host 実測（observable・2026-07-27）: user slice の階層版 `high 5302 / max 97189 / oom 34 / oom_kill 4` に対し `.local` は `high 5302 / max 0 / oom 0 / oom_kill 0`（同 slice の `cgroup.procs` は **0 行**・`nr_descendants` 43）。**観測できたのはカウンタの値だけ**: この層の `.local` は `max` 0 / `oom` 0 / `high` 5302。**意味づけは確定していない**が、判定を `max` / `oom` の直読で行う限り運用は変わらない（`oom_kill` について下に書くのと同じ扱い）。**`oom_kill` は層の弁別に使わない**——`cgroup.procs` が 0 行の層でも階層版は 4 を数えており、`.local` の 0 が「この層で起きていない」を意味するのか「この層に直接プロセスが無いから計上され得ない」を意味するのか、**観測だけでは弁別できない**（**規則 R3・deduced**: `.local` は自層に直接属するプロセスの kill しか数えない——凡例に登録。この解釈が正しくても誤っていても、**判定を `max` / `oom` で行う限り運用は変わらない**）。「いま throttle されているか」は `memory.pressure`（PSI）の `some/full avg10` を見る（`peak ≧ high` は「過去に一度でも触れた」しか言わない）。
...
- **`dmesg` / `journalctl -k` は本 fleet では読めない**（observable・2026-07-27 実測: `kernel.dmesg_restrict = 1` かつ実行 user が `adm` / `systemd-journal` 非所属＝`dmesg: read kernel buffer failed: Operation not permitted`）。**理由を「cgroup OOM は kernel log に出ないから」と書かないこと**——それは誤りで、memcg OOM も `Memory cgroup out of memory: Killed process …` を kernel ring buffer へ出す（＝権限のある host では補助 oracle になりうる）。本 fleet で空振りするのは**権限**であって kernel 挙動ではない（mechanism を verified 扱いで焼かない＝本 §6 の証拠種別規律）。なお **host が潤沢でも cgroup OOM は起きる**（実測時 125GiB 中 69GiB free で発生）——「host に空きがあるから OOM ではない」は**誤った消去法**。**dmesg に OOM 行が出ていても host OOM とは断定しない**: 上に書いたとおり memcg OOM も kernel ring buffer へ出るため、行文言（`Memory cgroup out of memory` か否か）で分かれる。**本 fleet では権限が無く未検証**なので、行文言を確認せずに host OOM と読まないこと。
- **`/var/crash` に CC の dump は無い**（observable・2026-07-28 実測: 存在するのは `_opt_google_chrome_chrome.1001.crash` の 1 件＝chrome 由来）。OOM killer が SIGKILL である以上 core dump も stderr も残らないと考えられる（**deduced**・未検証）。加えて CC は現在 **native binary**（`~/.local/share/claude/versions/`）で動くので `_usr_bin_node.*.crash` には現れない。**当該ディレクトリにあった node crash dump は codex 由来**（`ProcCmdline` が `node ~/.local/bin/codex`・SIGABRT・RSS 44MB）で **CC と無関係**だった＝「古い crash dump が新規記録を塞いでいる」という当時の作業仮説は**撤回済み**。
...
- **恒久既定は launcher が持つ**: `cc-session/scripts/cld` が **MemTotal 比例 + 下限 / 上限クランプ**で導出し（**具体値の SSOT は cld の定数ブロック**——ここへ数値を転記すると必ず drift するので書かない）、起動時に stderr 1 行（`cld: MemoryMax=… [auto: MemTotal …]`）で実効値と出所を可視化する。上書きは `CLD_MEMORY_MAX`（値）/ `CLD_MEMORY_MAX_PCT`（割合）、抑制は `CLD_MEMORY_QUIET`（真偽値・**保護不在に関する警告は一律抑制しない**＝上限なし / 防壁が bind しない / 平常 peak に届かない、のいずれも出る。個別列挙でなく規則で覚えること）。**既定変更が効くのは次回 cld 起動から**で、**いま走っている session は旧上限のまま**＝重い WF を回す前に上の `set-property` で個別に引き上げるか、実効値を起動時 1 行 / cgroup 直読で確認すること。**上限の撤廃はしない**——暴走 WF が host を巻き込むのを防ぐ防壁でもあるため。
- **per-scope 上限は fleet 総量を縛らない（運用側の手当てが要る）**: 単一 session の暴走への防壁であって、**同時稼働セッション数 × 上限の総和**は制約しない。**総和を比べる相手（分母）は MemTotal とは限らず `min(MemTotal, 上位 user slice の memory.max)`**（上の「層の弁別」で直読する。実測 host では MemTotal より 15GiB 低く、`memory.high` はさらに低い）——MemTotal を分母に数えると混雑を過小評価する。scribe の常態（admin + N worker + consult）で総和が MemTotal を超える構成になるなら手当てが要るが、**レバーは 2 本あり効く範囲が違う**（片方は混雑域で無効になるので、下げたつもりで放置しないこと）:
  ...
  - **下限より下が必要なら `CLD_MEMORY_MAX=<N>G` で明示値を与える**（明示値は下限/上限クランプを迂回する。1GiB 未満・100% 超・64bit 桁溢れ・不正形式は warn して自動導出へ倒れるので escape hatch の typo で起動が壊れることはない。**倒さずに「受理したうえで loud に警告」する保護不在が 2 種ある**——① 実 RAM 以上＝防壁が bind しない、② 実効値が CC の平常 peak 未満＝起動直後に OOM kill されうる〔`12G` を `1G` と打ち間違えた場合や、小 host で `<小さい割合>%` を指定した場合〕。**明示しても黙らない**ので、警告が出たまま運用しないこと）。**この 2 つと「倒す」側は失敗様式が別物なので混ぜないこと**: 倒す側（桁溢れ等）が防いでいるのは **systemd の property parse 失敗＝プロセスが一切起動しない**（**observable**・admin が sandbox 外の実機で確認・2026-07-27: wrap した値は byte 形 `18446744073709551700` も % 形も `Failed to parse MemoryMax=…: Numerical result out of range` で **rc=1**）、警告側の ② が防いでいるのは **parse を通ったうえでの実行時 OOM kill**（同実測で `1G` は **rc=0** で起動する＝parse は通る）。か、**同時本数を絞る**（総和が MemTotal を超える構成のまま運用しない）。
  - **下げたら効いたことを実効値で確認する**: 起動時 1 行（`cld: MemoryMax=… [auto: MemTotal …]`）に導出内訳が出る。総和超過が per-scope 上限で防げないことは変わらないが、**それが dmesg 側にどう現れるかは本 fleet では未検証**（上の空振り経路を参照）。
...
**証拠種別（本 §6 凡例）**: oom-kill 行・時刻一致・`memory.max` / `memory.peak` / `memory.events{,.local}` の実測はいずれも **observable（admin / worker 検証済み）**。本節の**手順**は observable のみに立脚する。**層の弁別に関する規則は最小限にし、すべて deduced タグ付きで下表へ登録する**（4 巡の gate で判明した失敗様式は「観測」ではなく「導出」側にあった＝機構の positive な一般規則を立てないのが本節の書き方）。**下表は層の弁別に関する規則だけを載せる**——本節の他の箇所にも機構由来の記述は残っており、それらはその場で deduced タグを付けて扱う。現在の登録は 3 つ:

| 規則 | 内容 | 支える observable / 注意 |
|---|---|---|
| **R1** | journal が名指す unit は「その subtree で kill が観測された層」で、祖先も同時に名指されうる | `app.slice`（`cgroup.procs` 0 行・上限未設定）と `-.slice` が名指されている実測と整合。**肯定的な意味づけは断定しない**——層の判定には使わない |
| **R2** | 上位 cgroup が binding なら子の上限を緩めても天井は動かない | cgroup v2 の階層制約。**「上位が binding か」の判定に unit 名を使わない**（`.local` 直読） |
| **R3** | `memory.events.local` は自層に直接属するプロセスの kill しか `oom_kill` に数えない | `cgroup.procs` 0 行の層で `.local` の `oom_kill` が 0 という observable と整合するが、整合は機構の証明ではない。**判定は `max` / `oom` で行うので R3 の真偽は運用に影響しない** |

いずれも本 fleet の worker sandbox（systemd/D-Bus 不可）では実挙動を確認できていない。**層の弁別に関する規則を足すときは必ずこの表へ 1 行として登録する**（層の弁別について無登録の一般規則を書かない）。なお `memory.high` が kill でなく throttle であることも同じ deduced 扱いで本文に明示してある。**mechanism を verified 扱いで焼かない**（この規律違反が本節の過去の誤り＝「cgroup OOM は kernel log に出ない」を生んだ）。
...
- **入口（2系統・sc-bs0 2026-07-03 grill 確定）**: 本フローの入口は2つ。**入口A（graph 駆動・従来）** = needs-user ラベルのタスクを admin が graph 処理するとき（上記発火条件で判定）。**入口B（ユーザー駆動）** = ユーザーが consult 起動を依頼したとき（`/scribe:consult`）。入口Bでは AI が spawn **前**に議題を **2段判定**する:
  ...
  判定が「決定を確定・記録したい」形のときは、**AI が facet 数判定で pre-bake 要否を決めて進む**（2026-07-25 裁定【聞かないこと】＝順序・選択肢の是認だけを求める問いは出さない。旧規約の 2 択質問「pre-bake 推奨ですが実行しますか？／plain consult をすぐ起動しますか？」は廃止）。**grill-issue が 1 件起票されることと WF のコストは事後に 1 行で開示する**（承認を求めるのではなく、決めて進んだ結果を可視化する）。**ユーザーが「pre-bake なしで」等を明示済みならその指定が優先**（判定を挟まず素 consult を即起動）。pre-bake 選択時は本フロー（下記 1.〜6.）にそのまま乗せる — grill-issue の起票も本フロー内（step 2＝brief 集約後）でその場で行う（`--context` は grill-issue 必須＝決定 handoff 先の bd notes を定める技術制約であり、この issue は「決定の記録先」そのもの。起票は admin 専有＝§3）。skill 側の手順は `skills/consult/SKILL.md` step 0（本節が規約 SSOT・skill は本文を転記しない）。決定経緯 = bd sc-bs0 notes（旧 regime sc-osn→sc-in9→sc-cuw では入口Bのルーティングが未検討で、pre-bake WF がユーザー駆動経路で構造的にバイパスされていた gap の解消）。
...
- **発火条件の縮小注記（引用の外・additive・2026-07-25 裁定）**: 上記 6 項の「**人間承認が必要な action**」は **3 クラス該当と snapshot-mismatch トリガ該当のみ**を指す（§5.4 canonical block: 消す / 出す / 使う ＋ §5.4 snapshot-mismatch トリガ＝3 クラスのいずれにも属さない機械 tamper 検知ゆえ 2026-07-25 裁定でも据置）。**様式とタイミング規律は不変**——front-load / バナーは**廃止でなく scope 縮小**であり、規約ファイル・全ホスト配布物・事前合意逸脱はもはや承認要求の対象ではない（AI 敵対 gate 通過をもって AI 判断で進む・§5.4）。引用本文（user 確定の verbatim）は 1 byte も改変していない。
...
- **可視性要件（user 裁定 2026-07-17・scriptorium orch-dsb6 追補）**: 承認要求は**存在自体が一目で分かる loud 表示**でなければならない（散文に埋め込まれた承認要求を user が読み落とす実害への追補＝2 項「定型で直接問う」の表示様式を確定する）。admin/consult が user 承認を求める**全場面**（3 クラス該当の確認・NEEDS-USER park からの確認・grill 裁定確認等）に適用:
...
> 一次出典: scriptorium orch-w3h4（user 裁定 2026-07-14・AskUserQuestion 一括承認・文面 verbatim 確定・3 層 codify〔scriptorium top-spec §5.4+CLAUDE.md / 本節 / global CLAUDE.md=uns 宛別便〕）/ bd sc-y28k（本反映 cell）/ scriptorium orch-dsb6（user 裁定 2026-07-17・可視性要件 (a)-(d) 追補・top-spec §5.4 / global CLAUDE.md と 3 層同時 codify）/ bd sc-9aqy（可視性要件の本反映 cell）/ scriptorium `orch-vhiu`（user 裁定 2026-07-25）——**本節の様式・タイミング規律は不変で、発火条件のみ 3 クラス該当へ縮小**（廃止ではない・反映 cell = bd `sc-o2nm`）。
...
- **scriptorium への作業依頼（handoff intake・`needs-orch`）**: cross-repo 調整を scriptorium（orchestrator）へ依頼するときは、自 `sc-` bead に平ラベル **`needs-orch`**（完全一致・regex 走査不可）を付ける。orchestrator が hydrate pull で拾う（**一報・連絡は不要＝bead が truth**）。**foreign 向け作法（本 bullet の各規則）は本 §8 が正本（operative）**であり、orchestrator 側の受信会計（検知線・coord bead lifecycle・受信 triage〔top-spec §1.1 の triage 表への入力〕）は scriptorium top-spec §5.1・§1.1 が持つ（役割分担・本文を相互に二重化しない＝本 bullet の判定基準は「foreign が貼るか決める」ための基準・§1.1 の表は「orchestrator が受領後どう捌くか」で別物・決定 = `orch-am1` grill 2026-07-08 人間裁定）。scribe は本規約を**機械 enforce しない**（advisory 運用＝§8 の他規約と同型。なお直前の federate-publish 規約は SSOT が orchestrator 側にある**逆向きの非対称**＝enforce しない点だけが共通で、正本の所在は規約ごとに異なる）。
  - **何が cross-repo か（判定基準・hybrid）**: **P1**=実装が自 repo の write 権の外に落ちる（自分では着地できない領域＝write-guard が機械的に止めるため迷う余地は生じにくい）∨ **P2**=他 repo に conformance（契約変更・規約追随）を強いる（**P2 のみ admin 判断**）。典型例 = 共有配布物（hooks / plugin / 全ホスト設定＝**旧 merge-gate カテゴリ語**の借用。**承認カテゴリとしては廃止された**〔§5.4・orch-vhiu 2026-07-25〕**が cross-repo 判定語としては存続**する）touch・他 repo との契約ファイル変更。**除外**=他 repo を read/参照/依存するだけ・並行進化（似た実装だが調整不要）は cross-repo でない。**曖昧時（非対称）**: P1 は機械捕捉ゆえ迷う余地なし・P2 で迷ったら貼って上げる（誤 handoff は orchestrator 一瞥で有限・上げ損ねは恒久 burial＝非対称）・純 in-repo は上げない。
...
- **admin 受信優先順位（何を先に捌くか・裁定-comm-protocol (b)・orch-thgx / orch-0yof ④）**: admin が同時に複数の入力（user・進行中の作業・orchestrator の下り便）を抱えたときの**捌く順序**は次で固定する——**① user 直接指示 > ② 現 atomic step の完了 > ③ orchestrator mailbox（`for:sc`）を cycle 区切りで消化**。すなわち user 指示は最優先で受けるが、orchestrator 由来の入力は**現在の atomic step を捨てて飛びつかない**（park-by-default の順序面＝上の bullet が「どう吸収するか（bead へ外部化してから triage）」を定め、本 bullet が「いつ捌くか（step 完了後・cycle 区切り）」を定める。二重化でなく同じ規律の別の軸）。さらに **全 orchestrator 依頼は bead 化して受ける**——relay（push 注入）は「**bead を見よ**」の**呼び鈴**であって依頼の実体ではない（上の「inject は呼び鈴のみ・中身は bead」の受信側適用形。呼び鈴だけで実体が bead に無いなら、実体を求めて bead へ差し戻す）。
  - **配送の機械面（受信規約の carrier・sc-b6w）**: 下り便（`for:sc` の open / in_progress bead・in_progress は sc-7ry4 で追加＝未配達 limbo の封鎖）は **2 つの配送点**が surface する——**SessionStart 配送点**（`scripts/hooks/session-start-mailbox-scan.sh`・bundle 境界で全件）と **UserPromptSubmit 中間配送点**（`scripts/hooks/user-prompt-mailbox-scan.sh`・毎 prompt で**新着のみ**・既報は再通知しない・TTL staleness-gate で間引く）。後者は**長寿命 admin session の滞留保険**（SessionStart だけでは bundle 境界まで下り便が滞留する＝`for:sc` 4 本が 2 日滞留した実 incident の恒久対策・裁定-delivery-guarantee②）。いずれも **direct read（hydrate 禁止）**で、surface されたものを admin が上記優先順位で triage する（surface は「気づき」であって「即実行」の指示ではない）。
...
- **courier 受け bead 規約（delivered+acked 2 段の受信側・grill E 裁定・courier orch-rejt）**: courier 便を受けた admin は自台帳に**受け bead を立て、便 id（`orch-xxxx` 等）を description / notes に明記**する（cross-ref＝orchestrator の acked scanner は「受け bead の実在 + 便 id」で受領を機械判定する。既存 bead へ編入して受ける場合も便 id 明記は同じ）。**初回返信で自 backlog との整合を返す**——① 衝突する既存 task（serialize 集合・dup・依存）② 組み込み位置 ③ 見積り——を受け bead notes へ front-load する（write-isolation 下で orchestrator は read のみ＝clarification 往復ができない・needs-orch 必須 3 の受信側対応物）。機械保証は **delivered（mailbox 配送・上記配送点）+ acked（受け bead の実在）の 2 段**まで（acked scanner は orchestrator 実装＝scribe scope 外）で、**discharged（実装完了の突合）は手動敵対照合を維持**する。**射程明確化**: 「per-bead ack write 禁止」（上記 heartbeat bullet）は **foreign 台帳への書き戻し禁止**の意であり、**受信側の自台帳 ack（受け bead）は禁止対象外**（write-isolation と受信会計は両立する）。**orch への整合返信・discharge 報告は open bead の notes に置く**——closed bead への append は updated_at が前進しても orchestrator の open-scan から**構造的に不可視**（W7 実測 2026-08-05: closed 受け皿へ置いた回答に『未着』照会が 2 度来た・open bead への再掲で解消）。受け bead を close するときは返信要素を **open の epic / 集約 carrier bead へ再掲してから** close する。
...
- **危険域の実測値（数値そのもの）の転写点は §0 admin core 行のみ（sc-aprn D5）**: 長寿命 admin session の context 危険域の実測値は、scribe 側では **§0 の admin core「寿命(1) 危険域」行だけ**が保持する（転写元 = orchestrator top-spec §1.1・core 行に出典注記あり）。本節本文へは**二重化しない**（数値の複数転写点は drift 源。core 行と本節の非対称は意図的で、危険域は「発火前に知らないと手遅れ」型ゆえ boot 配達側〔core〕が数値を持つ）。
