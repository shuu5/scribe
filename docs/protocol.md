...
- **危険域の実測値（~240k）の転写点は §0 admin core 行のみ（sc-aprn D5）**: 長寿命 admin session の context 危険域の実測値は、scribe 側では **§0 の admin core「寿命(1) 危険域」行だけ**が保持する（転写元 = orchestrator top-spec §1.1・core 行に出典注記あり）。本節本文へは**二重化しない**（数値の 3 重化は drift 源。core 行と本節の非対称は意図的で、危険域は「発火前に知らないと手遅れ」型ゆえ boot 配達側〔core〕が数値を持つ）。
