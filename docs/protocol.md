...
- **WF 外側ループ 4 点（再走可否の機械判定・resume 再走・escalation 計数・値札記録）は機械実装が SSOT（sc-46kv・M0 外側 4 裁定）**: `scripts/scribe-wf-rerun-judge.sh`（judge = cap 契約 v2 返り値から blocking 級 drop を機械判定〔rc 0=受容 / 1=再走要 / 2=判定不能→再走側〕・count/marker = 行頭 `[WF-RERUN--<id>]` 逐次記帳＝自動再走は 1 回まで・**usage header が規約本文で resume 手順と効かない 3 条件を含む**）+ `scripts/scribe-wf-cost.sh`（probe/record = `[SCRIBE-WF-COST v1]` 値札を bd notes へ・**invocation 単位＝resume は run record を上書きし累計にならない**）。数値閾値・severity 集合は env/args が SSOT（P1 実分布待ち・doc に決め打ちしない）。
