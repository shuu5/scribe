...
1. **worker 報告の監査**: worker の self-report（commit hash / WF 返り値 JSON / `receivedArgs` / self-test 結果 / 成文化・実装した内容）を読み、args 解決の成否（`parseFailed`/`receivedArgs`）と self-test の pass を一次確認する。
   ...
   - **worker 申告の bats not-ok は sandbox 環境汚染でありうる（「退行ゼロ」の根拠に使えない・sc-von0 gate-4 実測）**: worker の bats 実行は既定で OS sandbox 内（§2 sandbox 節）＝環境要因の not ok が混入する（実測 = worker 申告 1464 ok / 114 not ok に対し admin 実測 1578 ok / 0 not ok / rc=0・母集団 1464+114=1578 の**完全一致**＝114 件全てが sandbox 由来で repo の状態ではない）。**退行ゼロ判定は admin の sandbox 外実測を truth** とし、worker 申告の not ok 数を単独で「repo の退行」と読まない（母集団一致の確認で環境要因と切り分ける）。
...
5. **push 前に origin URL 健全性を verify（汚染なら fail-loud + 復元）**（bd un-1n1）: squash merge / GitHub push の **前に** リポの `origin` URL が spawn 時 canonical のままかを照合する。worktree は anchor と `.git/config`（remotes）を共有するため、worker が origin を mutate すると anchor+全 worktree の origin が壊れ push が破綻する（2026-06-16 un-v5x 実害＝134s タイムアウト+gh が known host 無しと誤認）。汚染を検知したら **fail-loud し、canonical へ復元してから** push する（汚染したまま push しない）。
   - **中間 commit message の leak は push した瞬間に確定する（merge 前の作法・sc-i2kc 追加3・doobidoo `ff25f370`）**: commit message に leak（実名 / 絶対パス / private 値）が残ったまま push すると GitHub の PR commit 一覧に載り**取り消せない**（3 クラス (b) 出す の一方向性）。branch **未 push** の段階なら `git reset --soft <base>` + 単一 commit へ畳むことで露出ゼロで除去できる——**tree 差分が空**であることを確認して lossless を証明する（未 push の soft-reset は履歴改変〔(a) 消す〕に当たらない）。worker に history rewrite を強いない。
   - **leak battery の第 4 系統（名前ベースでは原理的に捕まらない型・実装 = bd sc-pion）**: 現行 3 系統（実名 word-boundary / deploy path / 短名列挙）は全て「名前」を見る——**commit id 等の不透明 hex token は名前検査で捕まらない**。added-line の hex token を `git cat-file -t` に通し**本 repo で解決しないものを RED** とする 1 コマンドの機械判定を第 4 系統として leak battery に含める（規律本文 = 本項が SSOT・実装所有 = bd sc-pion）。
...
- **発報条件**: busy regex 不一致 × commit / file 活動なし × 終端ラベルなし が**約 20 分継続**。
...
> 一次出典: bd sc-5cbx notes [ADMIN-STEERING-2] / [SALVAGE-RESPAWN]・bd sc-rr8b（silent stall 実害の移送元・sc-i2kc へ統合）・doobidoo `85f8b5b4`（横断 incident 教訓）・orch 側 frame 分類の SSOT 予定 = orch_frame_classify.sh（bd sc-koox と相互参照）。
